...
| 43 | Content MCP tools for grounded roles (Phase 7) | MCP content tools (`content.retrieve`, `content.list_sources`, `content.get_document`) are granted by default to the C2-grounded roles `coder` and `reviewer` (`def.mcpAllow` + `ALLOWED_TOOLS_PER_ROLE`); analyzer/pr keep existing grants; context/policy have NO MCP tool names (decision #41 holds). Existing seeded policy DBs report drift until `sor:policy reconcile` (capabilitySnapshot seeds `doc.mcpAllow` from `def.mcpAllow`). |

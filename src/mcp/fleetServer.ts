...
function runGhApi(args: string[]): Promise<GhApiResult> {
	return new Promise((resolve) => {
		...
		proc.on("error", (err) => {
			...
			resolve({
				stdout: "",
				stderr: `gh failed to start: ${err.message}`,
				exitCode: 127,
			});
		});
		proc.on("close", (code) => {
...
// Fails closed by default: only roles listed here get any MCP tools at all.
// Intentionally limited to `analyzer` (read-only issue context via
// get_issue/get_issue_comments) and `pr` (create_pr/get_checks, the only role
// that needs to touch GitHub via `gh api`). planner/coder/tester/reviewer are
// intentionally excluded — they only need the bash/file toolset from
// `src/fleet/tools/`, not GitHub API access. If a role needs a new MCP tool,
// add it explicitly here rather than widening the allowlist by default.
const ALLOWED_TOOLS_PER_ROLE: Record<string, string[]> = {
	analyzer: ["get_issue", "get_issue_comments"],
	pr: ["create_pr", "get_checks"],
};

function isToolAllowedForRole(
...
async function handleGetIssueComments(
	args: Record<string, unknown>,
): Promise<unknown> {
...
async function handleGetChecks(
	args: Record<string, unknown>,
): Promise<unknown> {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
	const allTools = [
		{
			name: "get_issue",
			description: "Get issue title, body, and labels",
			inputSchema: GET_ISSUE_SCHEMA,
		},
		{
			name: "get_issue_comments",
			description: "Get issue comments",
			inputSchema: GET_ISSUE_COMMENTS_SCHEMA,
		},
		{
			name: "create_pr",
			description: "Create a pull request",
			inputSchema: CREATE_PR_SCHEMA,
		},
		{
			name: "get_checks",
			description: "Get check runs for a commit ref",
			inputSchema: GET_CHECKS_SCHEMA,
		},
	];

	return {
		tools: allTools.filter((t) => isToolAllowedForRole(SESSION_ROLE, t.name)),
	};
});
...
export { ALLOWED_TOOLS_PER_ROLE, handleToolCall, isToolAllowedForRole };

...
function reqBoolean(
  ...
  if (typeof value !== "boolean") {
    throw new Error(`Tool ${tool}: missing or invalid boolean argument '${key}'`);
  }
  return value;
}
...
function reqRecord(
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error(`Tool ${tool}: missing or invalid object argument '${key}'`);
  }
...
async function createRun(args: Record<string, unknown>): Promise<{ run_id: string; created_at: string }> {
...
async function updateRunStatus(args: Record<string, unknown>): Promise<{ updated: boolean }> {
...
async function logAgentAction(args: Record<string, unknown>): Promise<{ action_id: string }> {
...
async function finalizeRun(args: Record<string, unknown>): Promise<{ finalized: boolean }> {
...
async function queryCostByRole(args: Record<string, unknown>): Promise<CostByRoleRow[]> {
  const fromDate = reqDate(args, "from_date", "query_cost_by_role").toISOString();

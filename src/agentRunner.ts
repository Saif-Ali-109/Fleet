...
import { closeSync, existsSync, fstatSync, mkdirSync, openSync, readSync, readFileSync } from "node:fs";
...
import { withProviderFallback, type ProviderAttemptOutcome } from "./providers/registry.ts";
...
import { assertGeminiQuotaConfiguration, geminiQuotaConfig, geminiRateLimitWaitMs } from "./gemini/quotaConfig.ts";
import { RPD_EXHAUSTED, parseRateLimitSwitch } from "./fleet/quotaSignals.ts";
import { emitQuotaEvent } from "./fleet/quotaEvents.ts";
import { MANAGER_ID, newRequestId, type RequestIdentity } from "./telemetry.ts";
...
function quotaCoordinator(): GeminiQuotaCoordinator {
  ...
  const configKey = JSON.stringify(Object.entries(limits).sort(([a], [b]) => a.localeCompare(b)));
...
export async function runWorker(
  ...
  try {
    ...
    workerDbId = await hireWorker(role, ctx.provider ?? "gemini", policy.model, workforcePolicy);
  } catch (err) {
   ...
   */
  const geminiChainWalk = async (): Promise<ProviderAttemptOutcome<ParsedStream>> => {
    ...
    for (;;) {
      ...
      for (let i = 0; i < models.length; i++) {
        ...
        const parsed = await spawnOnce("gemini", role, task, ctx, opts.extraTask, tracePath, opts, model, resumeMessagesPath);
        last = parsed;
        ...
        geminiAttempts.push({
          ...
          ok,
          ...(ok || parsed.errorMsg === undefined ? {} : { error: parsed.errorMsg }),
          provider: "gemini",
        });
        if (ok) {
          if (everBlocked.delete(model)) {
            emitQuotaEvent({ type: "model_recovered", role, provider: "gemini", model });
          }
        ...
        const temporary = parsed.errorMsg === "GEMINI_RATE_LIMIT_WAIT_EXCEEDED" || parsed.errorMsg === "GEMINI_RATE_LIMIT_WAIT_CEILING";
        return { model: lastModel, ok: false, value: parsed, error: parsed.errorMsg, stopFallback: temporary };
      }
      if (rpdDead.size === models.length) {
        emitQuotaEvent({ type: "all_models_exhausted", role, provider: "gemini", models });
        ...
        if (!resumed) {
          ...
          return { model: lastModel, ok: false, value: last, error: "aborted by user", stopFallback: true };
        }
        resumeMessagesPath = existsSync(checkpointPath) ? checkpointPath : undefined;
        zeroWaitCycles = 0;
        continue;
      }
    ...
    return { model: lastModel, ok: false, value: last, error: last?.errorMsg ?? "all configured Gemini models quota exhausted" };
  };
    ...
    async (provider) => {
      ...
      if (provider !== "gemini") {
        ...
        for (const model of [policy.model]) {
          const parsed = await spawnOnce(provider, role, task, ctx, opts.extraTask, tracePath, opts, model);
          last = parsed;
          const ok = !parsed.sawError && !abortRequested && parsed.text.trim().length > 0;
          if (ok) return { model: parsed.model ?? model, ok: true, value: parsed };
          if (parsed.errorMsg === RPD_EXHAUSTED) continue;
          const temporary = parsed.errorMsg === "GEMINI_RATE_LIMIT_WAIT_EXCEEDED" || parsed.errorMsg === "GEMINI_RATE_LIMIT_WAIT_CEILING";
          return { model: parsed.model ?? model, ok: false, value: parsed, error: parsed.errorMsg, stopFallback: temporary };
        }
        return { model: last?.model ?? policy.model, ok: false, value: last, error: last?.errorMsg ?? "all configured Gemini models quota exhausted" };
      }
  ...
  const firstGeminiAttempt = mappedWalkAttempts.findIndex((a) => a.provider === "gemini");
    ...
    geminiAttempts.length === 0
      ...
      : firstGeminiAttempt === -1
        ...
        : [
            ...
            ...geminiAttempts,
            ...mappedWalkAttempts.slice(firstGeminiAttempt + 1).filter((a) => a.provider !== "gemini"),
          ];

  if (workerDbId) {
    try {
      await updateWorkerStatus(workerDbId, walk.ok ? "success" : "failed", tracePath);
    } catch {
      /* non-fatal */
    }
  }
    ...
    try {
      ...
      if (quota) {
        child.on("message", (message: unknown) => {
          ...
          if (
            ...
            m.attempt < 1 ||
            (workerSessionId !== undefined && m.sessionId !== workerSessionId) ||
          ...
          const result = quota.checkAndReserve({
            provider: "gemini",
            model: m.model,
            estimatedInputTokens: typeof m.estimatedInputTokens === "number" ? m.estimatedInputTokens : -1,
            maximumOutputTokens: typeof m.maximumOutputTokens === "number" ? m.maximumOutputTokens : -1,
          });
          if (!result.ok && result.waitMs !== undefined && result.waitMs > geminiRateLimitWaitMs()) {
            result.waitMs = 0;
          ...
          if (result.terminal) opts.onEvent?.({ t: "quota_exhausted", model: m.model, resetAt: result.resetAt, state: "quota_exhausted" });
        });
      }
      ...
      const job = {
        ...
        ctx: {
          ...
          ...(resumeFromPath !== undefined ? { resumeFrom: { messagesPath: resumeFromPath } } : {}),
        },
      };
      if (child.stdin) {
...
export function parseTrace(
  ...
  try {
    ...
    return { text: "", sessionID: null, tokens: zeroTokens(), costUsd: 0, sawError: false, tools: 0, models: 0, skills: 0, breakdown: {} };
  }
...
export function finalize(
  ...
  return {
    ...
    calls: { tools: s.tools, models: s.models, skills: s.skills, breakdown: s.breakdown },
    error,
    tracePath,
    startedAt,
    endedAt: Date.now(),
...
export const emptyStream = (): ParsedStream => ({
  text: "", sessionID: null, tokens: zeroTokens(), costUsd: 0, sawError: true,
  tools: 0, models: 0, skills: 0, breakdown: {},
});
...
export function aggregateAgentResults(results: AgentResult[]): AgentResult {
  ...
  for (const r of results) {
    ...
    for (const [k, v] of Object.entries(bd)) callsBreakdown[k] = (callsBreakdown[k] ?? 0) + v;
  }
  return {
    role: first.role,
    ...
    calls: { tools: callsTools, models: callsModels, skills: callsSkills, breakdown: callsBreakdown },
    error,

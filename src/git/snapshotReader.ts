...
export type SymbolKind = "class" | "interface" | "type" | "function" | "enum" | "default";
 ...
 */
export const SKELETON_CHAR_BUDGET = Number(process.env.SNAPSHOT_SKELETON_CHARS ?? 8_000);
...
const TEXT_EXTS = new Set([
  ".ts", ".tsx", ".ts", ".jsx", ".mjs", ".cjs", ".py", ".go", ".rs", ".java",
  ".cpp", ".cc", ".c", ".h", ".hpp", ".rb", ".php", ".sh", ".bash", ".zsh",
  ".md", ".json", ".toml", ".yaml", ".yml", ".html", ".css", ".scss",
]);
...
function skipTopDir(firstSegment: string): boolean {
  ...
  return firstSegment.startsWith(".") && firstSegment !== "." ? true : SKIP_DIRS.has(firstSegment);
}
...
function compilePatterns() {
  const patterns: { kind: SymbolKind; re: RegExp }[] = [
    { kind: "default", re: /^export\s+default\s+(?:class|function|interface|const|async\s+function|async\s+function\s*\*)/ },
    { kind: "class", re: /^(?:export\s+(?:default\s+)?|export\s+default\s+)?class\s+[A-Za-z_$][\w$]*[\s<{]/ },
    { kind: "interface", re: /^(?:export\s+)?interface\s+[A-Za-z_$][\w$]*[\s<{]/ },
...
export function symbolName(kind: SymbolKind, header: string): string {
  const m =
    header.match(/(?:class|interface|type|enum|function)\s+([A-Za-z_$][\w$]*)/)
    ?? header.match(/export\s+default\s+(?:class|function|interface|const)\s+([A-Za-z_$][\w$]*)/)
...
async function listTracked(dir: string): Promise<string[] | null> {
  try {
    const out = await git(["ls-files", "-co", "--exclude-standard", "-z", "--max-count", String(MAX_FILES)], dir);
    if (!out) return [];
...
export async function buildSkeletonMap(worktreeDir: string): Promise<SkeletonMap> {
  const tracked = (await listTracked(worktreeDir)) ?? (await listFallback(worktreeDir));
  ...
  for (const f of scannable) {
    ...
    const skel: FileSkeleton = { path: f, languageHint: extHint(f), symbols: [], truncated: false };
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const { kind, re } of SYMBOL_PATTERNS) {
        if (re(line) !== -1) {
          ...
          if (!name) continue;
          skel.symbols.push({ name, kind, line: i + 1, header, params: symbolParams(header) });
          break;
        }
      }
...
export async function readSelectedFileSymbols(worktreeDir: string, filePath: string): Promise<string> {

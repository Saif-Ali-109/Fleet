 ...
 */
function provisionFleetSkills(_repoDir: string, _worktreeDir: string): void {
...
export async function hasCommits(h: WorktreeHandle): Promise<boolean> {
	const out = await git(
		["rev-list", "--count", `${h.baseBranch}..HEAD`],
		h.worktreeDir,
	);
	return Number(out) > 0;
}
...
export async function changedFiles(h: WorktreeHandle): Promise<string[]> {
	const out = await git(
		["diff", "--name-only", `${h.baseBranch}...HEAD`],
		h.worktreeDir,
	);
 ...
 */
export async function pruneOldRunDirs(
	runsRoot: string,
	maxAgeDays = 7,
	...
	try {
		...
		errors.push(
			`readdir ${runsRoot}: ${e instanceof Error ? e.message : String(e)}`,
		);

...
import { ApiHandlers, type WebhookResponse, type WebhookHandler } from "./api.ts";
export type { WebhookResponse, WebhookHandler };
...
async function listLiveModels(provider: ProviderName): Promise<string[]> {
  return new Promise<string[]>((resolve) => {
    const timer = setTimeout(() => resolve([]), HEARTBEAT_MS); // Reusing HEARTBEAT_MS for consistency, could be separate
...
export class WebDashboard {
  ...
  constructor(
    ...
    try {
      ...
      this.combinedHtml = template.replace("<!-- INJECT_CLIENT_JS -->", `<script>${clientJs}</script>`);
    } catch (err) {
  ...
  pushAgentEvent(role: Role, event: Record<string, unknown>): void {
    ...
    if (snap.agentEvents[role]) {
      ...
      if (snap.agentEvents[role].length > MAX_CHUNKS) snap.agentEvents[role].shift();
    }
    this.lastEventId += 1;
  ...
  pushQuotaEvent(event: QuotaEvent): void {
    this.lastEventId += 1;
    this.broadcast(this.lastEventId, "quota_event", { type: "quota_event", event });
  }
  ...
  private snapshot(dash?: DashboardState | null): {
    ...
    return {
      dash: dash ?? this.apiSnapshot.dash, // Use provided dash or fallback
      outputs: this.apiSnapshot.outputs,
      agentEvents: this.apiSnapshot.agentEvents,
      gh: this.apiSnapshot.gh,
      notice: this.apiSnapshot.notice,
      nextScanAt: this.apiSnapshot.nextScanAt,
      runActive: this.apiSnapshot.runActive,
      queueMode: this.apiSnapshot.queueMode,
      provider: this.apiSnapshot.provider,
      stopRequested: this.apiSnapshot.stopRequested,
      paused: this.apiSnapshot.paused,
      errorLog: this.apiSnapshot.errorLog
    };
  }
  ...
  private handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (path === "/api/provider") {
      this.sendJson(res, 200, { provider: this.apiHandlers.provider, providers: [...PROVIDER_NAMES] });
      return;
    }
    if (path === "/api/events") {
    ...
    if (path === "/api/model-limit-error") {
      this.sendJson(res, 200, { ok: true, errorLog: this.apiHandlers.errorLog });
      return;
    }
  ...
  // Getter for API handlers state (for SSE and other internal use)
  private get apiSnapshot() {
    return this.apiHandlers.getSnapshot();
  }
}
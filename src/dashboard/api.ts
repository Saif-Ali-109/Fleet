...
export class ApiHandlers {
	...
	guardMutation(req: IncomingMessage, res: ServerResponse): boolean {
		...
		if (origin !== undefined) {
			...
			try {
				...
				this.sendJson(res, 403, {
					ok: false,
					error: "forbidden: bad Origin header",
				});
				return false;
			}
		...
		if (hostName !== "127.0.0.1" && hostName !== "localhost") {
			this.sendJson(res, 403, {
				ok: false,
				error: "forbidden: bad Host header",
			});
			return false;
		}
	...
	handleStart(req: IncomingMessage, res: ServerResponse): void {
		...
		req.on("end", () => {
			...
			const provider =
				...
				(PROVIDER_NAMES as readonly string[]).includes(rawProvider)
			...
			if (!this.onStartRequest) {
				...
				this.sendJson(res, 200, {
					ok: false,
					error: "no start handler registered",
				});
				return;
			}
	...
	handleModels(req: IncomingMessage, res: ServerResponse): void {
		...
		req.on("end", () => {
			...
			if (
				typeof role !== "string" ||
				!["analyzer", "planner", "coder", "tester", "reviewer", "pr"].includes(
					role as Role,
				)
			) {
			...
			try {
				...
				this.sendJson(res, 500, {
					ok: false,
					error:
						"failed to persist model overrides: " +
			...
			void this.observedModelList(provider).then((_available) => {
				// Note: Broadcast is handled by the dashboard core
			});
		});
		req.on("error", () => {
	...
	handleProvider(req: IncomingMessage, res: ServerResponse): void {
		...
		req.on("end", () => {
			...
			if (this.runActive) {
				this.sendJson(res, 409, {
					ok: false,
					error: "a run is in progress; cannot change provider",
				});
				return;
			}

...
export function makeOnEvent(
	...
	return (ev: Record<string, unknown>) => {
		if (scoutTracker.observe(role, ev)) {
			void logLine(
				ctx.rootDir,
				`[scout] invoked by ${role} (call ${scoutTracker.total}, ${scoutTracker.countFor(role)}/${role})`,
			);
		}
		if (ev.t === "quota_exhausted") {
			pushNotice?.(
				`Gemini model ${String(ev.model ?? policyModel)} quota_exhausted until ${new Date(Number(ev.resetAt ?? Date.now())).toISOString()}`,
			);
		}
		if (ev.t === "tool_call") {

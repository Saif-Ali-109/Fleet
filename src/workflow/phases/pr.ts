...
import type { AgentResult, RunContext, RolePolicy, Role } from "../../types.ts";
import type { RunStatus, RunSummary } from "../../orchestrator.ts";

export interface PrPhaseOpts {
  ctx: RunContext;
  wt: WorktreeHandle;
  runAgent: (role: Role, phase: "pr", task: string, policy: RolePolicy) => Promise<AgentResult>;
...
export async function runPrPhase(opts: PrPhaseOpts): Promise<PrPhaseResult> {
  const { ctx, wt, runAgent, setPhase, pushState, finalize, makeSummary, agents } = opts;

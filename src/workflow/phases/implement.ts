...
import { EXCLUDE_ARTIFACTS, execErrorText, runCoder } from "../coder.ts";
...
const MAX_REVIEW_DIFF_CHARS = Number(
	process.env.MAX_REVIEW_DIFF_CHARS ?? 25_000,
);
...
export async function runImplementPhase(
	...
	const implTask = (feedback: string | undefined): string => {
		...
		if (feedback)
			lines.push("", "FEEDBACK FROM REVIEWER (auto-fix round):", feedback);
		return lines.join("\n");
	};
	...
	for (let iter = 1; iter <= 1 + AUTO_FIX_MAX_ROUNDS; iter++) {
		...
		if (runId) {
			const lastFailed = await getLastFailedStep(runId, "coder").catch(
				() => null,
			);
		...
		for (const role of implRoles) {
			...
			if (!runId) {
				...
				if (!res.ok) {
					...
					return {
						ok: false,
						summary: makeSummary("failed", res.error ?? `${role} failed`),
					};
				}
				continue;
			}
			...
			if (!ok) {
				...
				return {
					ok: false,
					summary: makeSummary("failed", error ?? `${role} failed`),
				};
			}
		}
		...
		if (!r.ok) {
			...
			return {
				ok: false,
				summary: makeSummary("failed", r.error ?? "reviewer failed"),
			};
		}
		if (!ctx.dryRun) {
			...
			if (isRequestChanges || !isApprove) {
				const reviewerFeedback = isRequestChanges
					? [verdict?.rationale, ...(verdict?.blockingIssues ?? [])]
							.filter(Boolean)
				...
				await logLine(
					ctx.rootDir,
					"reviewer rejected after final auto-fix round",
				);
	...
	if (!approved) {
		...
		return {
			...
			summary: makeSummary(
				"failed",
				"could not reach an approved implementation",
			),
		};
	}

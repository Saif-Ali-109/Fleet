...
import type { AgentResult, FixSpec, RunContext, RolePolicy, Role } from "../../types.ts";
...
export async function runAnalyzePhase(opts: AnalyzePhaseOpts): Promise<AnalyzePhaseResult> {
  const { ctx, runAgent, setPhase, pushState, finalize, makeSummary, runId, sorEmit } = opts;
  ...
  if (!a.ok) {
    ...
    return { ok: false, summary: makeSummary("failed", a.error ?? "analyzer failed") };
  }
  if (runId) {
  ...
  if (!fixSpec) {
    ...
    return { ok: false, summary: makeSummary("failed", "analyzer did not return a valid FixSpec JSON") };
  }

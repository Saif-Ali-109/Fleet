...
describe("canonicalJson", () => {
  ...
  it("preserves array order and sorts each element object's keys", () => {
    const json = canonicalJson({ items: [{ id: 2, name: "b" }, { id: 1, name: "a" }] });
    expect(json).toBe('{"items":[{"id":"2","name":"b"},{"id":"1","name":"a"}]}');
  });
...
describe("signEvent", () => {
  ...
  it("changes when the event content changes", () => {
    ...
    const changed = signEvent("secret", GENESIS_HASH, makeEvent({ tool_name: "write" }));
  ...
  it("canonicalizes before hashing (key order does not matter)", () => {
    ...
    expect(signEvent("secret", GENESIS_HASH, e1)).toBe(signEvent("secret", GENESIS_HASH, e2));
  });
});
...
describe("eventToRecord / normalizeEvent round-trip", () => {
  ...
  it("defaults payload to {} and coerces created_at to ISO when given a Date", () => {
    const e = normalizeEvent({ event_type: "phase", actor: "system", created_at: new Date(0) });
  ...
  it("coerces run_id numbers to strings", () => {
    const e = normalizeEvent({ event_type: "wakeup", actor: "daemon", run_id: 42 });
...
describe("truncateValue", () => {
  ...
  it("recursively truncates nested objects/arrays to a total character budget", () => {
    const obj = { a: "x".repeat(80), b: { c: "y".repeat(80), d: ["z".repeat(80)] } };
  ...
  it("truncates arrays element-by-element to the cap", () => {
    const out = truncateValue(["a".repeat(60), "b".repeat(60)], 100) as string[];
...
describe("normalizeEvent validation", () => {
  ...
  it("throws when event_type is not a valid SorEventType", () => {
    expect(() => normalizeEvent({ event_type: "bogus", actor: "manager" })).toThrow(/event_type/);
  });
  ...
  it("throws when payload is not an object", () => {
    expect(() => normalizeEvent({ event_type: "phase", actor: "manager", payload: "nope" })).toThrow(
      /payload/,
    );
  });
  ...
  it("throws on invalid created_at", () => {
    expect(() =>
      normalizeEvent({ event_type: "phase", actor: "manager", created_at: "not-a-date" }),
    ).toThrow(/created_at/);
  });

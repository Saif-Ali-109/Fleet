...
import { type SorEvent, TOOL_INPUT_CAP } from "../events.ts";
import {
	normalizeHookLine,
	normalizeTraceEvent,
	readEventFile,
...
describe("normalizeTraceEvent", () => {
	it("maps a step_finish-ish object to a wakeup event with tokens payload", () => {
		...
		expect(ev?.payload.tokens).toEqual({
			input: 10,
			output: 5,
			reasoning: 2,
			total: 17,
	...
	it("returns null for unrecognized event types", () => {
		expect(
			normalizeTraceEvent({ type: "text", part: { text: "hello" } }),
		).toBeNull();
		expect(normalizeTraceEvent({ type: "step_start" })).toBeNull();
		expect(
			normalizeTraceEvent({ type: "error", part: { error: "boom" } }),
		).toBeNull();
	});
	...
	it("truncates long tool_input against TOOL_INPUT_CAP", () => {
		...
		const ev = normalizeTraceEvent({
			...
			part: {
				type: "tool_call",
				tool: "bash",
				state: { status: "completed", input: long, output: "ok" },
			},
		});
		expect(ev?.tool_name).toBe("bash");
		expect(typeof ev?.tool_input).toBe("string");
		const toolInput = ev?.tool_input;
		expect(typeof toolInput === "string" ? toolInput.length : 0).toBe(
			TOOL_INPUT_CAP,
		);
	});
});

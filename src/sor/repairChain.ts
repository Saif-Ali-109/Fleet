...
try {
  ...
  if (rows.length === 0) {
    console.log(`no audit events found for key_id="${currentKeyId}" — nothing to repair`);
    code = 0;
  } else {
    interface Update {
      seq: number;
  ...
  console.error("[sor] repair failed:", err instanceof Error ? err.message : String(err));
  code = 1;
}
await pool.end();
process.exit(code);

  ...
  async exec(input, ctx): Promise<ToolResult> {
    ...
    for (const file of walkFiles(ctx.worktreeDir)) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        results.push(`${relative(ctx.worktreeDir, file)}:${i + 1}: ${(lines[i] as string).trim()}`);
...
export const globTool: ToolImpl = {
  ...
  async exec(input, ctx): Promise<ToolResult> {
    ...
    try {
      ...
      return { ok: false, error: `invalid glob: ${err instanceof Error ? err.message : String(err)}` };
    }
    const results: string[] = [];
    let truncated = false;
    for (const file of walkFiles(ctx.worktreeDir).sort()) {
      const rel = relative(ctx.worktreeDir, file).split("\\").join("/");
      const candidate = pattern.includes("/") ? rel : rel.split("/").pop() ?? rel;

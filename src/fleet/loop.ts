...
import { RATE_LIMIT_SWITCH_PREFIX, RPD_EXHAUSTED, rateLimitSwitchError } from "./quotaSignals.ts";
import { newRequestId, type RequestIdentity, type TelemetryEvent, type TelemetryKind } from "../telemetry.ts";
...
export type WireEvent =
  | { t: "init"; role: Role; model: string; provider: ProviderName; sessionId: string; managerId?: string; runId?: string; workerId?: string }
  | { t: "text"; part: { text: string } }
  | { t: "tool_call"; name: string; input: unknown }
  | { t: "tool_result"; name: string; ok: boolean; ms: number; bytesOut: number; exitCode?: number }
...
export interface RunAgentOpts {
  ...
  ) => Promise<{ ok: boolean; reservationId?: string; waitMs?: number; terminal?: boolean; block?: string; error?: string }>;
}
...
function isTransientLlmError(err: unknown): boolean {
  ...
  if (msg.startsWith(RATE_LIMIT_SWITCH_PREFIX) || msg === RPD_EXHAUSTED) return false;
...
export function classifyProviderError(err: unknown): {
  ...
  return { rateLimited, ...(httpStatus !== undefined ? { httpStatus } : {}), ...(dimension ? { blockedDimension: dimension } : {}) };
}
...
export function ollamaMaxRetries(raw: string | undefined = process.env.OLLAMA_MAX_RETRIES): number | null {
  if (raw === undefined || raw.trim() === "") return null;
  const value = Number(raw);
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("OLLAMA_MAX_RETRIES must be a non-negative integer (0 = no retries after a transient failure; unset = unlimited)");
  }
  return value;
}
...
export async function createStreaming(
  ...
  const stream = (await withStallGuard(
    Promise.resolve(
      create({ ...opts, stream: true, stream_options: { include_usage: true } } as Parameters<CreateFn>[0], { signal: controller.signal }),
    ),
...
function writeCheckpoint(
  ...
  try {
    ...
    writeFileSync(
      tmpPath,
      JSON.stringify({ role, model, chainIndex: 0, messages, savedAt: new Date().toISOString() }),
    );
    renameSync(tmpPath, finalPath);
  } catch (err) {
    if (!checkpointWarnedRoles.has(role)) {
      checkpointWarnedRoles.add(role);
      console.warn(`[checkpoint] write failed for ${role}, continuing: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
...
export async function runAgent(opts: RunAgentOpts): Promise<RunAgentOutcome> {
  ...
  try {
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = opts.initialMessages ?? [
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        try {
          if (provider === "gemini") {
            if (!opts.reserve) throw new Error("Gemini quota coordinator unavailable");
            const estimatedInputTokens = Math.ceil(messages.reduce((n, m) => n + JSON.stringify(m).length, 0) / 4);
            const reservation = await opts.reserve(requestIdentity, estimatedInputTokens, opts.maxOutputTokens ?? 8192);
            const reserved = reservation.ok && Boolean(reservation.reservationId);
            emitTelemetry({
              t: "telemetry",
              ...
              ...(reservation.block ? { blockedDimension: reservation.block } : {}),
              status: reserved ? "reserved" : "rejected",
              ...(reservation.waitMs !== undefined ? { waitMs: reservation.waitMs } : {}),
            });
            if (!reserved) {
              if (reservation.ok) throw new Error("Gemini quota reservation missing reservationId");
              if (reservation.terminal) throw new Error(RPD_EXHAUSTED);
              if (reservation.block !== "rpm" && reservation.block !== "tpm") {
                throw new Error(reservation.error ?? "GEMINI_RATE_LIMIT_WAIT_EXCEEDED");
              }
              ...
              const modelSwitch: Omit<TelemetryEvent, "event"> & { event: TelemetryKind | "model_switch" } = {
                t: "telemetry",
                event: "model_switch",
                timestamp: new Date().toISOString(),
                ...requestIdentity,
              ...
              throw rateLimitSwitchError(reservation.block, reservation.waitMs ?? 0);
            }
          ...
          if (providerCallStarted) {
            emitTelemetry({
              ...
              ...(classification.blockedDimension ? { blockedDimension: classification.blockedDimension } : {}),
            });

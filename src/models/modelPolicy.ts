...
export function policyFor(
	...
	const rawEnv =
		process.env[`${role.toUpperCase()}_MODEL_${provider.toUpperCase()}`];
	const fromEnv =
		typeof rawEnv === "string" && rawEnv.length > 0 ? rawEnv : undefined;
	const chosen =
		overrides[provider]?.[role] ?? fromEnv ?? tierDefault ?? base.model;
...
export function allPolicies(
	providers: readonly ProviderName[] = PROVIDER_NAMES,
): RolePolicy[] {
...
export function getModelOverrides(): Partial<
	Record<ProviderName, Partial<Record<Role, string>>>
> {
...
export function loadModelOverrides(modelsJsonPath: string): void {
	...
	try {
		parsed = JSON.parse(readFileSync(modelsJsonPath, "utf8")) as Record<
			string,
			unknown
		>;
	} catch {
	...
	const legacyKeys = Object.keys(parsed).filter(
		(key) => !(PROVIDER_NAMES as readonly string[]).includes(key),
	);
	if (legacyKeys.length > 0 && !warnedLegacyOverrideKeys) {
		warnedLegacyOverrideKeys = true;
		console.warn(
			`[models] discarding legacy v1 override keys: ${legacyKeys.join(", ")}`,
		);
	}

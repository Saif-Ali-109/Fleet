...
import { PauseManager, resumeFromPause, setActiveResumeHandler, reloadApiKeyEnv } from "./fleet/pauseManager.ts";
...
export { commitMessageFor, collapseConsecutiveModels };
...
export async function runOrchestrator(
  ...
  const writeResultFile = async (summary: RunSummary): Promise<void> => {
    try {
      ...
      console.warn(`[result] result.json write skipped (non-fatal): ${String(e)}`);
    }
  };
  ...
  const unsubscribeQuota = onQuotaEvent((ev: QuotaEvent): void => {
    if (ev.type === "model_switch") {
      const wait = ev.waitMs > 0 ? ` (wait ~${Math.round(ev.waitMs / 1000)}s)` : "";
    ...
    console.error(msg);
    void logLine(ctx.rootDir, msg);
    void logLine(ctx.rootDir, `[quota] run paused — completed roles stay completed, pipeline position preserved`);
  ...
  setActiveResumeHandler((): boolean => {
    ...
    if (keysReloaded.length > 0) {
      void logLine(ctx.rootDir, `[quota] resume: reloaded ${keysReloaded.join(", ")} from .env`);
    }
    invalidateProviderClients();
    try {
      resetGeminiQuotaCoordinator();
    } catch (e) {
      console.warn(`[quota] coordinator reset failed (non-fatal): ${String(e)}`);
    }
    pm.exitPause(web);
    const delivered = requestQuotaResume();
    void logLine(ctx.rootDir, `[quota] run resumed${delivered ? "" : " (no parked worker walk)"} — restarting ${role} from its checkpoint`);
    void sorEmit(ctx, {
  ...
  try {
    ...
    const analyzeResult = await runAnalyzePhase({
      ctx, runAgent, setPhase, pushState, finalize, makeSummary, runId, sorEmit,
    });
    if (!analyzeResult.ok) return analyzeResult.summary;
    const { fixSpec } = analyzeResult;

    const planResult = await runPlanPhase({
      ctx, fixSpec, runAgent, setPhase, pushState, finalize, makeSummary, runId, sorEmit, skeleton,
    });
    ...
    const prResult = await runPrPhase({
      ctx, wt, runAgent, setPhase, pushState, finalize, makeSummary, agents,
    });

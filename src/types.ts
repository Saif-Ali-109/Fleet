...
export interface AgentResult {
	...
	}[]; // each model/provider tried, in order
	...
	/** Exit code of the most recent `bash` tool call in the trace, if any. NOTE: this is
	 *  "whatever bash call ran last" — a worker may run further bash calls (e.g. `git commit`)
	 *  after the command a caller actually cares about, so prefer `bashCommands` when you need
	 *  the outcome of a *specific* command. */
	lastBashExitCode?: number;
	/** Every `bash` tool call in the trace, in order, paired with its exit code (if known).
	 *  Used by the tester workflow to gate pass/fail on the actual test command's exit code
	 *  instead of on the LLM's self-reported summary text or an unrelated later bash call. */
	bashCommands?: Array<{ command: string; exitCode?: number }>;
	error?: string;
	tracePath: string; // .runs/<id>/traces/<role>.jsonl
	startedAt: number;
	endedAt: number;
	/** Per-role call counters: tool invocations, model API calls, skill loads. */
	calls?: {
		tools: number;
		models: number;
		skills: number;

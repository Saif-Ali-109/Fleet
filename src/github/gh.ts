...
/** Sanity ceiling for `--limit`: `gh issue list` pages internally via GraphQL when the
 *  requested limit exceeds a single page, so raising this is enough to stop silently
 *  dropping issues in repos with more than 100 open — we just need a ceiling so a
 *  runaway repo can't make one scan pull an unbounded number of issues. */
const ISSUE_LIST_LIMIT = 1000;

/** List open issues of a repo (up to `ISSUE_LIST_LIMIT`), sorted ascending by number.
 *  Logs a warning if the ceiling is hit so a truncation is never silent. Rejects on
 *  bad repo/auth. */
export async function listOpenIssues(
	...
	if (items.length >= ISSUE_LIST_LIMIT) {
		console.warn(
			`[gh] listOpenIssues: hit the ${ISSUE_LIST_LIMIT}-issue ceiling for ${repo}; some open issues may not be visible to the daemon this scan.`,
		);
	}
...
export async function hasOpenPrForBranch(
	...
	if (!Array.isArray(j)) {
		throw new Error(
			`gh pr list returned an unexpected payload: ${JSON.stringify(j).slice(0, 200)}`,
		);
	}
...
export async function fetchIssue(
	...
	return {
		...
		labels: Array.isArray(j.labels)
			? j.labels.map((l: { name: string }) => l.name).filter(Boolean)
			: [],
...
async function ghBestEffort(args: string[], what: string): Promise<void> {
	try {
		...
		console.warn(
			`[gh] ${what} failed (non-fatal): ${err?.stderr?.trim() || err?.message || String(e)}`,
		);
	}
}
...
export async function startDeviceLogin(): Promise<DeviceLoginRequest> {
	...
	if (!j.device_code) {
		throw new Error(
			String(j.error_description ?? "device login failed to start"),
		);
	}
	return {
		...
		verificationUri: String(
			j.verification_uri ?? "https://github.com/login/device",
		),
...
export async function pollDeviceToken(
	...
	while (Date.now() - started < timeoutMs) {
		...
		if (j.error) {
			throw new Error(
				String(j.error_description ?? `device login failed: ${j.error}`),
			);
		}
	}
	...
	await new Promise<void>((resolve, reject) => {
		...
		child.on("close", (code) => {
			if (code === 0) resolve();
				...
				reject(
					new Error(stderr.trim() || `gh auth login exited with code ${code}`),
				);
		});
...
export async function ghAuthInfo(): Promise<GhAuthInfo> {
	try {
		...
		if (!username)
			return { ok: false, error: "gh is not signed in — run `gh auth login`" };

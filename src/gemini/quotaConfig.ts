...
function validLimit(value: unknown): value is GeminiQuotaLimit {
  ...
  return ["rpm", "tpm", "rpd"].every((key) => typeof v[key] === "number" && Number.isInteger(v[key]) && v[key] > 0);
}

export function parseGeminiQuotaLimits(raw: string | undefined): GeminiQuotaLimits {
  if (!raw?.trim()) return {};
  let parsed: unknown;
  try {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("GEMINI_QUOTA_LIMITS must be an object");
...
export function geminiQuotaConfig(overrides?: GeminiQuotaLimits): GeminiQuotaLimits {
  const env = parseGeminiQuotaLimits(process.env.GEMINI_QUOTA_LIMITS ?? process.env.GEMINI_QUOTA_OVERRIDES);
  return { ...GEMINI_QUOTA_DEFAULTS, ...env, ...overrides };
}

export function geminiRateLimitWaitMs(raw = process.env.GEMINI_RATE_LIMIT_WAIT_MS): number {
  if (raw === undefined || raw.trim() === "") return 120_000;
  const value = Number(raw);
  if (!Number.isInteger(value) || value < 0) throw new Error("GEMINI_RATE_LIMIT_WAIT_MS must be a non-negative integer");
  return value;
}
...
export function geminiModelChain(role: Role, primary: string): { model: string; fallbacks: string[] } {
  return { model: primary, fallbacks: parseGeminiRateLimitModels().filter((model) => model !== primary) };
}

export function warnIfGeminiPoolUnset(logger: (msg: string) => void = console.warn): void {
  if (parseGeminiRateLimitModels().length === 0) {
    logger("[quota] GEMINI_RATE_LIMIT_MODELS unset — every role runs a single-model chain (no fallback on limits)");
  }
}
...
export function validateGeminiQuotaConfiguration(
  roles: readonly Role[] = ["analyzer", "planner", "coder", "tester", "reviewer", "pr"],
  ...
  for (const role of roles) {
    ...
    for (const model of new Set([policy.model, ...policy.fallbacks])) {
      ...
      if (!model.trim()) errors.push({ role, model, message: "model id is empty" });
      else if (!limit) errors.push({ role, model, message: "missing quota limits" });
      else if (!validLimit(limit)) errors.push({ role, model, message: "quota limits must be positive integers" });
    }
  }
  return errors;
}
...
export function assertGeminiQuotaConfiguration(
  ...
  if (errors.length) throw new Error(`Invalid Gemini quota configuration:\n${errors.map((e) => `${e.role}/${e.model}: ${e.message}`).join("\n")}`);
}
...
export function assertGeminiModelChainConfiguration(
  ...
  if (errors.length) throw new Error(`Invalid Gemini model chain configuration:\n${errors.map((e) => `${e.role}/${e.model}: ${e.message}`).join("\n")}`);
}

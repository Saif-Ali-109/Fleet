...
describe("Gemini quota configuration", () => {
	...
	it("ships the Gemini free-tier model defaults", () => {
		...
		expect(GEMINI_QUOTA_DEFAULTS["gemini-3.1-flash-lite"]).toEqual({
			rpm: 15,
			tpm: 250_000,
			rpd: 500,
		});
	...
	it("keeps configured model ids exact", () => {
		...
		expect(
			validateGeminiQuotaConfiguration(["coder"], GEMINI_QUOTA_DEFAULTS),
		).toEqual([
			{
				role: "coder",
				model: "gemini-3.5-flash-lite-v2",
	...
	it("accepts arbitrary exact model ids through JSON overrides", () => {
		expect(
			parseGeminiQuotaLimits('{"custom":{"rpm":1,"tpm":2,"rpd":3}}'),
		).toEqual({
		...
		expect(
			validateGeminiQuotaConfiguration(
				["coder"],
				geminiQuotaConfig({ custom: { rpm: 1, tpm: 2, rpd: 3 } }),
			),
		).toEqual([]);
	});
	...
	it("reports missing limits for configured model overrides", () => {
		setModelOverride("coder", "custom", "gemini");
		expect(
			validateGeminiQuotaConfiguration(["coder"], {
...
describe("Gemini rate-limit model chains", () => {
	...
	afterEach(() => {
		...
		for (const role of ROLES)
			delete process.env[`${role.toUpperCase()}_MODEL_GEMINI`];
	});

	function poolLimits() {
		return Object.fromEntries(
			POOL.map((model) => [model, { rpm: 1, tpm: 2, rpd: 3 }]),
		);
	}
	...
	it("parses GEMINI_RATE_LIMIT_MODELS into an ordered deduped pool", () => {
		...
		process.env.GEMINI_RATE_LIMIT_MODELS =
			" gemini-2.5-flash ,, gemma-4-31b-it , gemini-2.5-flash ,";
		expect(parseGeminiRateLimitModels()).toEqual([
			"gemini-2.5-flash",
			"gemma-4-31b-it",
		]);
	...
	it("accepts a fully populated chain configuration", () => {
		...
		expect(() =>
			assertGeminiModelChainConfiguration(ROLES, poolLimits()),
		).not.toThrow();
	});
	...
	it("requires every <ROLE>_MODEL_GEMINI var", () => {
		...
		const missing = [
			{ role: "analyzer", model: "", message: "missing ANALYZER_MODEL_GEMINI" },
		];
		expect(validateGeminiModelChainConfiguration(ROLES, limits)).toEqual(
			missing,
		);
		...
		expect(() => assertGeminiModelChainConfiguration(ROLES, limits)).toThrow(
			/missing ANALYZER_MODEL_GEMINI/,
		);
	});
	...
	it("flags chain models lacking quota entries", () => {
		...
		expect(
			validateGeminiModelChainConfiguration(["coder"], poolLimits()),
		).toEqual([
			{
				role: "coder",

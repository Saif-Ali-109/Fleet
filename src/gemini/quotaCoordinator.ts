...
export class GeminiQuotaCoordinator {
	...
	constructor(
		...
		this.limits = Object.fromEntries(
			Object.entries(limits).map(([model, limit]) => [model, { ...limit }]),
		);
	...
	reserve(request: ReservationRequest): ReservationResult {
		if (this.failed)
			return {
				ok: false,
				block: "error",
				error: "Gemini quota coordinator is unavailable",
			};
		try {
			...
			if (
				...
				!limit ||
				!Number.isInteger(request.estimatedInputTokens) ||
			...
			if (!state) throw new Error(`Model state not found: ${request.model}`);
			...
			if (tokens > limit.tpm)
				return {
					...
					block: "tpm",
					waitMs: 0,
					error: "Reservation exceeds model TPM limit",
				};
			...
			const rpmWait =
				state.requests.length >= limit.rpm && oldest
					? oldest.at + WINDOW - now
					: 0;
			const usedTokens = state.requests.reduce((sum, r) => sum + r.tokens, 0);
			const tpmWait =
				usedTokens + tokens > limit.tpm && oldest
					? oldest.at + WINDOW - now
					: 0;
			if (rpmWait > 0 || tpmWait > 0)
				return {
					ok: false,
					block: rpmWait >= tpmWait ? "rpm" : "tpm",
					waitMs: Math.max(rpmWait, tpmWait),
				};
			const reservation = {
				id: `${request.model}:${++this.sequence}`,
				at: now,
				tokens,
			};
	...
	recordUsage(
		...
		_actualInputTokens: number,
		_actualOutputTokens: number,
	): void {
	...
	modelState(
		model: string,
		now = this.clock(),
	): { quotaExhausted: boolean; resetAt?: number } {
		const s = this.state.get(model);
		if (!s) throw new Error("Unknown Gemini model");
		if (s.exhaustedUntil !== undefined && now >= s.exhaustedUntil)
			s.exhaustedUntil = undefined;
		return {
			quotaExhausted: s.exhaustedUntil !== undefined,
...
function validLimit(limit: GeminiQuotaLimit): boolean {
	return (
		Number.isInteger(limit.rpm) &&
		limit.rpm > 0 &&
		Number.isInteger(limit.tpm) &&
		limit.tpm > 0 &&
		Number.isInteger(limit.rpd) &&
		limit.rpd > 0
	);
}

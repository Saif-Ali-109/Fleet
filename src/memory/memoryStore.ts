...
/**
 * MEMORY.txt is the durable, cross-run knowledge file (decision 7). The orchestrator is the
...
export async function readMemory(rootDir: string): Promise<string> {
	const p = resolveManagerPath(rootDir, "MEMORY.txt");
...
export async function appendRunOutcome(
	...
	const current = existsSync(p) ? await readFile(p, "utf8") : "# MEMORY.txt\n";

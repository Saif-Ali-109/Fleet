...
export function parseWorkerJob(raw: string): WorkerJob {
  ...
  if (c.maxOutputTokens !== undefined && (typeof c.maxOutputTokens !== "number" || !Number.isInteger(c.maxOutputTokens) || c.maxOutputTokens <= 0)) {
  ...
  if (c.resumeFrom !== undefined) {
    if (typeof c.resumeFrom !== "object" || c.resumeFrom === null || Array.isArray(c.resumeFrom)) {
    ...
    if (typeof r.messagesPath !== "string" || r.messagesPath.length === 0) {
      throw new Error("job.ctx.resumeFrom.messagesPath must be a non-empty string");
    }
...
async function run(): Promise<number> {
  ...
  let initialMessages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] | undefined;
  if (job.ctx.resumeFrom) {
    try {
      const parsed: unknown = JSON.parse(readFileSync(job.ctx.resumeFrom.messagesPath, "utf8"));
      const msgs = (parsed as { messages?: unknown } | null)?.messages;
      if (!Array.isArray(msgs)) throw new Error("checkpoint has no messages array");
      initialMessages = msgs as OpenAI.Chat.Completions.ChatCompletionMessageParam[];
    } catch (err) {
      return failOut(`resume checkpoint unreadable: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  const outcome = await runAgent({
    ...
    identity: job.ctx.managerId && job.ctx.runId && job.ctx.workerId
      ? {
      ...
      : undefined,
    reserve: provider === "gemini"
      ? (identity: RequestIdentity, estimatedInputTokens, maximumOutputTokens) => new Promise((resolve) => {
          const requestId = identity.requestId;
        const unavailable = (detail: string): { ok: false; error: string } => ({
          ok: false,
        ...
        const handler = (message: unknown): void => {
          ...
          if (m.type !== "quota_reserve_result" || m.requestId !== requestId) return;
          cleanup();
          resolve((m.result && typeof m.result === "object" ? m.result : { ok: false, error: "invalid quota response" }) as { ok: boolean; waitMs?: number; terminal?: boolean; error?: string });
        };
        ...
        try {
          process.send(
            ...
            (error) => {
              if (error) failUnavailable(`reservation request failed: ${error.message}`);
            },
          );
        } catch (error) {
          failUnavailable(`reservation request failed: ${error instanceof Error ? error.message : String(error)}`);
        }
      })
    : undefined,
  });

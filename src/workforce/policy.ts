...
function normalize(raw: Partial<WorkforcePolicy>): WorkforcePolicy {
  ...
  return {
    ...
    auto_hire_roles: Array.isArray(raw.auto_hire_roles) ? raw.auto_hire_roles : base.auto_hire_roles,
    gate_hire_roles: Array.isArray(raw.gate_hire_roles) ? raw.gate_hire_roles : base.gate_hire_roles,
    deny_hire_roles: Array.isArray(raw.deny_hire_roles) ? raw.deny_hire_roles : base.deny_hire_roles,
    authorized_roles: Array.isArray(raw.authorized_roles) ? raw.authorized_roles : base.authorized_roles,
  };
}

export function loadPolicy(pathArg?: string): WorkforcePolicy {
  const candidate = pathArg?.length ? pathArg : process.env.WORKFORCE_POLICY_PATH;
  if (!candidate?.length) {
    return defaultPolicy();
  }
  try {
    const resolved = path.resolve(candidate);
    const parsed = JSON.parse(readFileSync(resolved, "utf8")) as Partial<WorkforcePolicy>;

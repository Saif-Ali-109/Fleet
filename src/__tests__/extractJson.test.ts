...
describe("extractJson", () => {
	...
	it("handles escaped quotes inside string values", () => {
		const result = extractJson<{ msg: string }>('{"msg": "say \\"hi\\" now"}');
		expect(result).toEqual({ msg: 'say "hi" now' });
	});

	it("handles backslash escapes inside string values", () => {
		const result = extractJson<{ msg: string }>('{"msg": "a\\\\b\\\\c"}');
  ...
  "summary": "fix the crash"}`;
	...
	describe("truncation handling (some providers cap output tokens)", () => {
		...
		it("salvages an object truncated mid-string-value by appending a closing quote + brace", () => {
			...
			const result = extractJson<{ reason: string }>('{"reason": "the');
		...
		it("salvages a nested object missing both inner and outer braces", () => {
			...
			const result = extractJson<{ a: { b: string } }>('{"a": {"b": "x"');
		...
		it("salvages an object whose array value is truncated mid-string", () => {
			...
			const result = extractJson<{ items: string[] }>('{"items": ["a", "b');

...
} from "../dashboard/webDashboard.ts";
...
describe("dashboard", () => {
	describe("newDashboardState", () => {
		...
		describe("GET /api/models (live list + static fallback)", () => {
			...
			async function setup(provider: ProviderName = "gemini") {
				...
				if (!info) throw new Error("dashboard failed to start");
				baseUrl = info.url;
			}
	...
	describe("renderDashboard quota notice", () => {
		it("renders the latest quota event as a single status line", () => {
			const d = newDashboardState("run-1", "owner/repo", 7);
			d.quotaNotice =
				"coder: gemini-3.5-flash rate limited (rpm) → gemma-4-31b-it";
			const out = renderDashboard(d);
			expect(out).toContain(
				"⚠ coder: gemini-3.5-flash rate limited (rpm) → gemma-4-31b-it",
			);
		...
		it("renders exhaustion notices prominently until overwritten or the next run", () => {
			const d = newDashboardState("run-1", "owner/repo", 7);
			d.quotaNotice =
				"all Gemini models RPD exhausted for coder — change your API key";
			const out = renderDashboard(d);
			expect(out).toContain(
				"⚠⚠ all Gemini models RPD exhausted for coder — change your API key",
			);
	...
	describe("formatAgentEvent (embedded JS)", () => {
		...
		const match = src.match(/function formatAgentEvent\(ev\) \{[\s\S]*?^\t\}/m);
		if (!match)
			throw new Error("Could not extract formatAgentEvent from source");
		...
		const errMatch = src.match(/function fmtErr\(v\) \{[\s\S]*?^\t\}/m);
		if (!errMatch) throw new Error("Could not extract fmtErr from source");

		const esc = (s: unknown) =>
			String(s).replace(/&/g, "&").replace(/</g, "<").replace(/>/g, ">");
		// Use Function constructor instead of eval for safer dynamic evaluation
		const fmtErr = new Function(`${errMatch[0]}\nreturn fmtErr;`)() as (
			v: unknown,
		) => string;
		const fn = new Function(
			"esc",
			"fmtErr",
			`${match[0]}\nreturn formatAgentEvent;`,
		) as (
	...
	describe("outputs / agentEvents isolation", () => {
		it("keeps pushOutput strings out of agentEvents and vice versa", () => {
			...
			expect(
				internal.agentEvents.planner.some((e) => typeof e === "string"),
			).toBe(false);
			expect(internal.outputs.planner.every((c) => typeof c === "string")).toBe(
				true,
			);
		});
	});
	...
	describe("embedded page transcript rendering (incremental append)", () => {
		...
		it("pushAgentEvent appends a single event instead of calling the full renderAgentEvents rebuild", () => {
			const fn = src.match(
				/function pushAgentEvent\(role, ev\) \{[\s\S]*?\n\t\}/,
			);
			expect(fn).not.toBeNull();
			const body = fn?.[0];
		...
		it("appendAgentEvent inserts via insertAdjacentHTML rather than rebuilding innerHTML", () => {
			const fn = src.match(
				/function appendAgentEvent\(role, ev\) \{[\s\S]*?\n\t\}/,
			);
			expect(fn).not.toBeNull();
			expect(fn?.[0]).toContain("insertAdjacentHTML");
		});
	...
	describe("call counters (embedded JS)", () => {
		...
		const extractFn = (name: string, args: string) => {
			const pattern = `function ${name}\\(${args}\\) \\{[\\s\\S]*?^\\t\\}`;
			const m = src.match(new RegExp(pattern, "m"));
		...
		it("card() emits call ticks before the done-only cost/tokens meta", () => {
			...
			if (!m) throw new Error("Could not extract card from source");
			const body = m[0];
			expect(body).toMatch(/const calls = fmtCalls\(a\.calls\);/);
			const callsIdx = body.indexOf("const calls = fmtCalls(a.calls);");
		...
		it("renderAgents() renders the session totals strip behind a truthy guard", () => {
			...
			expect(body).toContain(
				"dash.totals && (dash.totals.tools || dash.totals.models)",
			);
		});
	});
	...
	describe("web dashboard layout (scroll containment)", () => {
		...
		it("mobile media query restores natural page scrolling", () => {
			...
			if (!m) throw new Error("Could not extract media query from source");
			const block = m[0];
	...
	describe("quota events (SSE + client rendering)", () => {
		...
		it("toast auto-dismisses after ~6 seconds", () => {
			...
			expect(fn).not.toBeNull();
			expect(fn?.[0]).toMatch(/setTimeout\(\(\) => \{[\s\S]*?\}, 6000\)/);
		});

		it("persistent red banner carries the exact exhaustion message", () => {
			expect(src).toMatch(
				/var QUOTA_EXHAUSTED_MSG =\n\s*"All Gemini models RPD exhausted — change your API key\. Run paused\.";/,
			);
			const fn = src.match(
				/function showQuotaExhausted\(text\) \{[\s\S]*?\n\t\}/,
			);
			expect(fn).not.toBeNull();
		...
		it("browser Notification fires only when already granted, inside try/catch (never prompts)", () => {
			...
			expect(body).toMatch(
				/try \{[\s\S]*?Notification[\s\S]*?\} catch \(_\) \{\}/,
			);
		...
		it("renders the banner and toast container elements in the page HTML", () => {
			...
			expect(src).toContain(
				'<span class="ghb-title" id="quotabanner-text"></span>',
			);
	...
	describe("quota pause/resume (P-quota)", () => {
		...
		describe("POST /api/resume route", () => {
			...
			it("exposes paused:false in /api/state until setPaused flips it", async () => {
				...
				expect(
					(
						(await (await fetch(`${baseUrl}api/state`)).json()) as {
							paused: boolean;
						}
					).paused,
				).toBe(false);
				dashboard.setPaused(true, "paused for quota");
				const snap = (await (await fetch(`${baseUrl}api/state`)).json()) as {
					paused: boolean;
					notice: string | null;
				};
		...
		it("renders the resume button inside the quota banner", () => {
			expect(src).toContain(
				'<button id="resumebtn" type="button" hidden>✓ Changed — Resume</button>',
			);
		});
		...
		it("drives the banner from the server-driven snapshot.paused flag (documented choice)", () => {
			...
			expect(
				src.match(/function applyState\(s\) \{[\s\S]*?\n\t\}/)?.[0],
			).toContain("renderPause(s);");
		});

		it("carries the exact pause banner text", () => {
			expect(src).toMatch(
				/var PAUSED_BANNER_MSG =\n\s*"All models RPD exhausted — change GEMINI_API_KEY, then Resume";/,
			);
		});
	...
	describe("always-visible provider bar (CHANGE 1)", () => {
		...
		const sliceDiv = (from: number) =>
			src.slice(from, src.indexOf("</div>", from));
	...
	describe("two-stream live transcript (CHANGE 2)", () => {
		...
		it("one shared renderer feeds BOTH sinks (toolsstream + transcript tab)", () => {
			...
			const body = fn?.[0];
			if (!body)
				throw new Error("Could not extract appendAgentEvent from source");
			expect(
				body.match(/renderAgentEvent\(ev\)/g)?.length,
	...
	describe("follow-scroll manager (CHANGE 4)", () => {
		...
		const computeNextStick = new Function(
			`var FOLLOW_RELEASE_PX = 40, FOLLOW_RESTICK_PX = 8; ` +
				src.match(
					/function computeNextStick\(stick, scrollTop, scrollHeight, clientHeight\) \{[\s\S]*?^\t\}/m,
				)?.[0] +
	...
	describe("onLoad boot resilience (CHANGE 5)", () => {
		...
		it("runs the /api/state + SSE bootstrap outside safe(), after all chunks", () => {
			const fn = src.match(/function onLoad\(\) \{[\s\S]*?\n\t\}/);
			if (!fn) throw new Error("Could not extract onLoad from source");

...
const {
	...
	normalizeWatchedSlug,
	SHUTDOWN_EXIT_CODE,
	createSingleIssueStopHandler,
} = await import("../index.ts");
process.argv = realArgv;

const makeController = (
	overrides: Partial<Parameters<typeof createShutdownController>[0]> = {},
) => {
...
describe("createShutdownController", () => {
	...
	it("first signal is graceful exactly once; second signal forces immediate exit 130", () => {
		...
		expect(consoleSpy).toHaveBeenCalledWith(
			"[shutdown] SIGTERM received again — forcing exit.",
		);
	});
...
describe("createSingleIssueStopHandler", () => {
	const makeDeps = (
		overrides: Partial<Parameters<typeof createSingleIssueStopHandler>[0]> = {},
	) => {
		const deps = {
	...
	it("first stop during a run requests stop and calls killWorkers exactly once", () => {
		...
		expect(deps.notify).toHaveBeenCalledWith(
			"Stop requested — aborted current issue (2 workers killed).",
		);
	});

	it("repeated stops while workers are dying stay idempotent and do not crash", () => {
		const { deps, handler } = makeDeps({
			killWorkers: vi.fn().mockReturnValueOnce(2).mockReturnValueOnce(0),
		});
		...
		expect(deps.notify).toHaveBeenNthCalledWith(
			1,
			"Stop requested — aborted current issue (2 workers killed).",
		);
		expect(deps.notify).toHaveBeenNthCalledWith(
...
describe("normalizeWatchedSlug vs webhook slug identity", () => {
	it("lowercases URL/slug inputs so mixed-case config matches lowercased webhook slugs", () => {
		expect(
			normalizeWatchedSlug("https://github.com/Saif-Ali-109/Demo-Repo.git"),
		).toBe("saif-ali-109/demo-repo");
		...
		if (!ev) throw new Error("parseIssueEvent returned null");
...
describe("quota-pause resume wiring (P-quota)", () => {
	...
	it("reloadApiKeyEnv applies only the two provider keys from .env and never throws on a missing file", async () => {
		const { reloadApiKeyEnv, resumeFromPause: _sameSeam } = await import(
			"../orchestrator.ts"
		);
		void _sameSeam;
		...
		try {
			...
			expect(reloadApiKeyEnv(root)).toEqual([
				"GEMINI_API_KEY",
				"OPENROUTER_API_KEY",
			]);

...
describe("modelPolicy (v2 override store: {provider:{role:id}})", () => {
	...
	describe("policyFor defaults per provider/role (SPEC §5 tier table)", () => {
		...
		it("defaults provider to gemini when omitted", () => {
			expect(policyFor("analyzer").model).toBe(
				policyFor("analyzer", "gemini").model,
			);
		});

		it("keeps the low reasoning variant for thinking roles only", () => {
			for (const role of STRONG_ROLES)
				expect(policyFor(role, "gemini").variant).toBe("low");
			for (const role of CHEAP_ROLES)
				expect(policyFor(role, "gemini").variant).toBeUndefined();
		});

		it("carries no redundant fallback when no override is set (ollama)", () => {
	...
	describe("env layer (SPEC D6: dashboard override > <ROLE>_MODEL_<PROVIDER> > tier default)", () => {
		...
		it("env var set → picked for that role+provider only", () => {
			...
			expect(policyFor("coder", "gemini").model).toBe(
				modelDefaults.gemini.coder,
			);
			expect(policyFor("tester", "ollama").model).toBe(
				modelDefaults.ollama.tester,
			);
		});

		it("env unset → tier default", () => {
			expect(policyFor("coder", "ollama").model).toBe(
				modelDefaults.ollama.coder,
			);
			expect(policyFor("analyzer", "ollama").model).toBe(
				modelDefaults.ollama.analyzer,
			);
		});
	...
	describe("gemini fallback chain (GEMINI_RATE_LIMIT_MODELS pool only)", () => {
		const ENV_KEYS = ["GEMINI_RATE_LIMIT_MODELS", "CODER_MODEL_GEMINI"];
		let savedEnv: Record<string, string | undefined>;
		...
		it("pool minus primary, order preserved (primary removed)", () => {
			process.env.GEMINI_RATE_LIMIT_MODELS = "a,b,c";
			process.env.CODER_MODEL_GEMINI = "b";
			const p = policyFor("coder", "gemini");
			expect(p.model).toBe("b");
			expect(p.fallbacks).toEqual(["a", "c"]);
		});

		it("primary not in pool → full ordered pool as fallbacks", () => {
			process.env.GEMINI_RATE_LIMIT_MODELS = "a,b,c";
			process.env.CODER_MODEL_GEMINI = "z";
			const p = policyFor("coder", "gemini");
			expect(p.model).toBe("z");
			expect(p.fallbacks).toEqual(["a", "b", "c"]);
		});

		it("no pool env → no fallbacks even when primary differs from the tier default", () => {
			process.env.CODER_MODEL_GEMINI = "custom-flash-id";
			expect(modelDefaults.gemini.coder).not.toBe("custom-flash-id");
			expect(policyFor("coder", "gemini").fallbacks).toEqual([]);
			expect(policyFor("analyzer", "gemini").fallbacks).toEqual([]);
		});

		it("dashboard override gets pool-minus-primary fallbacks (never the tier default)", () => {
			process.env.GEMINI_RATE_LIMIT_MODELS = "a,b,c";
			setModelOverride("analyzer", "b", "gemini");
		...
		it("openrouter still gets the implicit tier-default fallback when overridden", () => {
			process.env.GEMINI_RATE_LIMIT_MODELS = "a,b,c";
			setModelOverride("analyzer", "my-custom-id", "openrouter");
			const p = policyFor("analyzer", "openrouter");
			expect(p.model).toBe("my-custom-id");
			expect(p.fallbacks).toEqual([modelDefaults.openrouter.analyzer]);
		});
	});
	...
	describe("setModelOverride / getModelOverrides round-trip", () => {
		it("round-trips a per-provider role override", () => {
			...
			expect(getModelOverrides()).toEqual({
				openrouter: { analyzer: "custom-pro" },
			});
		...
		it("isolates overrides between providers and merges over defaults elsewhere", () => {
			...
			expect(policyFor("coder", "openrouter").model).toBe(
				modelDefaults.openrouter.coder,
			);
		});
		...
		it("getModelOverrides returns a defensive copy", () => {
			...
			if (snapshot.gemini) snapshot.gemini.pr = "mutated";
		...
		it("resetModelOverrides clears every provider", () => {
			...
			expect(Object.keys(getModelOverrides()).sort()).toEqual([
				"ollama",
				"openrouter",
			]);
			resetModelOverrides();
	...
	describe("save/load persistence to manager/models.json-style paths", () => {
		...
		it("regression: seeded-empty store keeps env above defaults across reboot", () => {
			...
			try {
				...
				expect(policyFor("analyzer", "ollama").fallbacks).toEqual([
					modelDefaults.ollama.analyzer,
				]);
			} finally {
	...
	describe("v1-key discard with single warning (log-once latch)", () => {
		it("discards legacy v1 per-backend keys and warns exactly once across loads", () => {
			...
			try {
				...
				expect(warn).toHaveBeenCalledWith(
					expect.stringContaining("opencode, claude, codex"),
				);
			} finally {
				warn.mockRestore();
		...
		it("does not warn on pure v2 files", () => {
			...
			writeFileSync(
				path,
				JSON.stringify({ ollama: { coder: "local-only" } }),
				"utf8",
			);
			const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
			try {
			...
			for (const provider of PROVIDER_NAMES) {
				for (const role of ROLES) {
					expect(
						policies.some(
							(p) =>
								p.role === role && p.model === policyFor(role, provider).model,
						),
					).toBe(true);
				}
			}

...
describe("getProviderDef", () => {
	it("exposes the exact SPEC §4 baseURLs", () => {
		...
		expect(registry.getProviderDef("openrouter").baseURL).toBe(
			"https://openrouter.ai/api/v1",
		);
	});

	it("defaults ollama to the local OpenAI-compatible endpoint", () => {
		expect(registry.getProviderDef("ollama").baseURL).toBe(
	...
	it("honors OLLAMA_BASE_URL at module load", async () => {
		...
		expect(fresh.getProviderDef("ollama").baseURL).toBe(
	...
	it("maps apiKeyEnv per provider (ollama needs none)", () => {
		...
		expect(registry.getProviderDef("openrouter").apiKeyEnv).toBe(
			"OPENROUTER_API_KEY",
		);
...
describe("getFleetProviders", () => {
	it("falls back to the default order when unset", () => {
		expect(registry.getFleetProviders()).toEqual([
			"gemini",
			"openrouter",
			"ollama",
		]);
	...
	it("skips invalid names with a warning and drops duplicates", () => {
		...
		expect(warn).toHaveBeenCalledWith(
			expect.stringContaining('skipping unknown provider "bogus"'),
		);
		warn.mockRestore();
	});
});
...
describe("hasProviderKey / providersWithKeys", () => {
	it("only ollama qualifies when no key env vars are set", () => {
		...
		expect(
			registry.providersWithKeys(["gemini", "openrouter", "ollama"]),
		).toEqual(["ollama"]);
	});
	...
	it("keeps fleet order once keys are present", () => {
		...
		expect(registry.providersWithKeys()).toEqual([
			"gemini",
			"openrouter",
			"ollama",
		]);
...
describe("withProviderFallback", () => {
	...
	it("returns the first successful attempt and stops the walk", async () => {
		process.env.GEMINI_API_KEY = "g";
		const order: string[] = [];
		const res = await registry.withProviderFallback(
			"coder",
			async (provider) => {
				order.push(provider);
		...
		expect(res.attempts).toEqual([
			{ provider: "gemini", model: "m-gemini", ok: true },
		]);
	});
		...
		const res = await registry.withProviderFallback(
			...
			async (provider) => {
				...
				if (provider === "openrouter")
					return { model: "m-or", ok: false, error: "503" };
	...
	it("honors an explicit provider list without consulting configured fallbacks", async () => {
		...
		const res = await registry.withProviderFallback(
			...
			async (provider) => {
				tried.push(provider);
				return { model: "gemini-model", ok: true, value: "done" };
			},
			["gemini"],
		);
		expect(res.provider).toBe("gemini");
	...
	it("fails clearly when an explicitly selected provider has no key", async () => {
...
describe("getClientForProvider", () => {
	it("memoizes one client instance per provider and constructs with the expected gemini opts", () => {
		...
		if (!first) throw new Error("ctorCalls[0] not set");

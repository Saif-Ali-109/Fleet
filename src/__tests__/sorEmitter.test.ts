...
describe("events.jsonl append ordering", () => {
  it("rapid interleaved emissions land in exact call-site order (after never precedes its before)", async () => {
    ...
    const observed = lines.map((l) => {
      const payload = (JSON.parse(l) as Record<string, unknown>).payload as Record<
        string,
        unknown
      >;
      return [String(payload.call_id), String(payload.phase)] as [string, string];
    });
    expect(observed).toEqual(issued);

    for (const callId of ["c1", "c2", "c3"]) {
      const phases = observed.filter(([id]) => id === callId).map(([, phase]) => phase);
      expect(phases[0]).toBe("before");
      expect(phases.indexOf("before")).toBeLessThan(phases.lastIndexOf("after"));
    }
  });
  ...
  it("a failing append does not break the queue for later appends", async () => {
    ...
    const lines = readFileSync(join(tmp, "events-good", "events.jsonl"), "utf8")
      ...
      .filter((l) => l.trim());
    expect(lines).toHaveLength(2);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("[sor] jsonl append skipped"));
  });
});

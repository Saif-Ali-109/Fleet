...
import { eventToRecord, normalizeEvent } from "../sor/events.ts";
...
const FIXTURE_PATH = join(
	import.meta.dirname,
	"fixtures/sor/historical-events.jsonl",
);
...
describe("SOR emitter parity with historical hook records", () => {
	it("tool_call before record has IDENTICAL field set AND key ordering vs the historical fixture", () => {
		...
		expect(
			Object.keys(emission.record.payload as Record<string, unknown>),
		).toEqual(Object.keys(histPayload));
	...
	it("tool_call after record has IDENTICAL field set AND key ordering vs the historical fixture", () => {
		...
		expect((emission.record.payload as Record<string, unknown>).phase).toBe(
			"after",
		);
...
describe("historical fixture replay through pure chain logic", () => {
	it("every tool_call line normalizes and hashes onto one unbroken chain", () => {
		...
		for (const ev of events) {
			...
			const manual = computeHash(
				key,
				prevHash,
				canonicalJson(eventToRecord(ev)),
			);
	...
	it("normalizeHookLine skips the session.created line without throwing", () => {
		...
		const created = lines.find(
			(l) => l.event_type === "session.created",
		) as Record<string, unknown>;
...
describe("normalizeEvent backend acceptance", () => {
	...
	it("still accepts legacy backends opencode/claude/codex and null", () => {
		...
		expect(
			normalizeEvent({ ...base, created_at: "2026-08-23T00:00:00Z" }).backend,
		).toBeNull();
	});
...
describe("non-fatal SOR writes", () => {
	...
	it("db append failure via a broken pool warns and never throws", async () => {
		...
		expect(() =>
			sink.toolResult("call_1", "bash", {}, "out", false, 5),
		).not.toThrow();
		await new Promise((r) => setTimeout(r, 30));
		expect(warn).toHaveBeenCalledWith(
			expect.stringContaining("[sor] db append skipped"),
		);
	});
...
describe("loop integration: sink called once per tool_call + once per tool_result", () => {
	it("forwards call id, tool name, input, full output, ok flag and duration", async () => {
		...
		try {
			...
			const resultArgs = calls[1]?.args as [
				string,
				string,
				unknown,
				string,
	...
	it("emitter writes two hook-shaped jsonl lines (before + after) into events.jsonl", async () => {
		...
		expect((after.payload as Record<string, unknown>).tool_output).toBe(
			"/x/a.ts\n/x/b.ts",
		);
...
describe("events.jsonl append ordering", () => {
	it("rapid interleaved emissions land in exact call-site order (after never precedes its before)", async () => {
		const eventsDir = join(tmp, "events");
		const sink = createSorEmitSink({
			runDir: tmp,
			role: "coder",
		...
		sink.toolCall("c1", "read", { path: "a.ts" });
		sink.toolCall("c2", "glob", { pattern: "*" });
		sink.toolResult("c2", "glob", { pattern: "*" }, "out2", true, 2);
		sink.toolResult("c1", "read", { path: "a.ts" }, "out1", true, 1);
		sink.toolCall("c3", "bash", { cmd: "ls" });
		sink.toolResult("c3", "bash", { cmd: "ls" }, "out3", true, 3);
		await new Promise((r) => setTimeout(r, 50));

		const lines = readFileSync(join(eventsDir, "events.jsonl"), "utf8")
			.split("\n")
			.filter((l) => l.trim());
		expect(lines).toHaveLength(issued.length);

		const observed = lines.map((l) => {
			const payload = (JSON.parse(l) as Record<string, unknown>)
				.payload as Record<string, unknown>;
			return [String(payload.call_id), String(payload.phase)] as [
				string,
				string,
			];
		});
		expect(observed).toEqual(issued);
		...
		for (const callId of ["c1", "c2", "c3"]) {
			...
			expect(phases.indexOf("before")).toBeLessThan(
				phases.lastIndexOf("after"),
			);
		}
	});

	it("a failing append does not break the queue for later appends", async () => {
		const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
		...
		expect(() => sink.toolCall("cx", "bash", {})).not.toThrow();

		const goodSink = createSorEmitSink({
			runDir: tmp,
			role: "coder",
			provider: "ollama",
			model: "m",
			sessionId: "s1",
			eventsDir: join(tmp, "events-good"),
			pool: null,
		});
		goodSink.toolCall("cy", "bash", {});
		goodSink.toolResult("cy", "bash", {}, "ok", true, 1);
		await new Promise((r) => setTimeout(r, 30));

		const lines = readFileSync(join(tmp, "events-good", "events.jsonl"), "utf8")
			.split("\n")
			.filter((l) => l.trim());
		expect(lines).toHaveLength(2);
		expect(warn).toHaveBeenCalledWith(
			expect.stringContaining("[sor] jsonl append skipped"),
		);
	});
});

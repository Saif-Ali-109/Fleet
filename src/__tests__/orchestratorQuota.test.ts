...
import { killActiveWorkers, requestQuotaResume, resetGeminiQuotaCoordinator, runWorker } from "../agentRunner.ts";
...
import { collapseConsecutiveModels, resumeFromPause, runOrchestrator, type WebFeed } from "../orchestrator.ts";
import {
  emitQuotaEvent,
...
const makeResult = (role: Role, text = "", ok = true, error?: string): AgentResult => ({
  role,
  ok,
  sessionID: null,
  model: "test-model",
...
const makeFeed = (): { feed: WebFeed; rec: FeedRecorder } => {
  const rec: FeedRecorder = { states: [], quotaEvents: [], notices: [], pauses: [] };
  const feed: WebFeed = {
...
const sorEvents = (): { event_type: string; payload: Record<string, unknown> }[] =>
...
describe("orchestrator quota event handling", () => {
  let consoleSpies: { warn: ReturnType<typeof vi.spyOn>; log: ReturnType<typeof vi.spyOn>; error: ReturnType<typeof vi.spyOn> };
  ...
  beforeEach(() => {
    ...
    vi.mocked(runCoder).mockImplementation(async () =>
      Promise.resolve({ ok: true, results: [makeResult("coder")], agentResult: makeResult("coder") } as never),
    );
  ...
  const stubHappyPath = (): void => {
    vi.mocked(runWorker).mockImplementation(async (role: Role) => {
      ...
      if (role === "pr") return makeResult(role, "Opened PR https://github.com/acme/widget/pull/99");
  ...
  it("announces model_switch on console, SESSION_LOG, SOR, live state and web feed", async () => {
    ...
    vi.mocked(runWorker).mockImplementation(async (role: Role) => {
      if (role === "analyzer") {
        ...
        emitQuotaEvent({ ...switchEvent, role: "tester", fromModel: "gemini-c", toModel: "gemini-d", waitMs: 0 });
    ...
    const notices = rec.states.map((s) => s.quotaNotice).filter((n): n is string => n !== undefined);
    ...
    expect(notices[notices.length - 1]).toBe("tester: gemini-c rate limited (rpm) → gemini-d");
    expect(consoleSpies.warn).toHaveBeenCalledWith(
      "[quota] tester: gemini-c rate limited (rpm) → switching to gemini-d",
    );
    const quotaWarns = consoleSpies.warn.mock.calls.map((c) => c[0] as string).filter((m) => m.includes("[quota]"));
  ...
  it("announces model_recovered with the switching-back message", async () => {
    ...
    expect(recov!.payload).toMatchObject({ role: "analyzer", provider: "gemini", model: "gemini-a" });
  ...
  it("pauses on all_models_exhausted instead of killing workers, and resume restarts the run", async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(sorEvents().some((e) => e.event_type === "run_paused")).toBe(true);
      });
      ...
      expect(rec.pauses.some((p) => p.paused && p.message?.includes("Resume"))).toBe(true);
      expect(consoleSpies.error).toHaveBeenCalledWith(expect.stringContaining("run paused"));
      expect(vi.mocked(logLine)).toHaveBeenCalledWith(ctx.rootDir, expect.stringContaining("run paused"));
      const ex = sorEvents().find((e) => e.event_type === "all_models_exhausted");
      expect(ex).toBeDefined();
      expect(ex!.payload).toMatchObject({ role: "tester", provider: "gemini", models: ["gemini-a", "gemini-b"] });
      expect(setIntervalSpy.mock.calls.filter(([, ms]) => ms === 300_000)).toHaveLength(1);
      ...
      expect(resumed!.payload).toMatchObject({ role: "tester", provider: "gemini" });
      const reminderIdx = setIntervalSpy.mock.calls.findIndex(([, ms]) => ms === 300_000);
  ...
  it("finalizes failed normally when the user Stops during the pause, writing result.json", async () => {
    ...
    const resultJson = JSON.parse(await readFile(join(ctx.runDir, "result.json"), "utf8")) as {
      status: string;
      failure?: string;
  ...
  it("collapses consecutive duplicate model ids when reporting fallbacks", () => {
    expect(collapseConsecutiveModels(["a", "a", "b", "b", "a"])).toEqual(["a", "b", "a"]);
  ...
  it("logs the fallback line with consecutive duplicates collapsed and skips single-model walks", async () => {
    ...
    vi.mocked(runWorker).mockImplementation(async (role: Role) => {
      ...
      if (role === "pr") return makeResult(role, "Opened PR https://github.com/acme/widget/pull/99");
      if (role === "reviewer") return makeResult(role, approveText);
      if (role === "planner") return { ...makeResult(role, PLAN_JSON), attempts: [{ model: "gemini-a", ok: true, provider: "gemini" }] };
      return makeResult(role);
    });
    const ctx = await makeCtx();
  ...
  it("unsubscribes when the run ends: post-run events reach nothing", async () => {
    ...
    emitQuotaEvent(switchEvent);
    expect(vi.mocked(appendAuditEvent).mock.calls.length).toBe(sorCallsAfterRun);
    expect(consoleSpies.warn.mock.calls.length).toBe(warnCallsAfterRun);
    expect(vi.mocked(logLine)).not.toHaveBeenCalledWith(ctx.rootDir, expect.stringContaining("[quota]"));
    expect(killActiveWorkers).not.toHaveBeenCalled();
  });
  ...
  it("fails fast before any side effects when a role chain var is missing on a real run", async () => {
    ...
    try {
      ...
      expect(summary.status).toBe("failed");
      expect(summary.failure).toContain("Invalid Gemini model chain configuration");
  ...
  it("assertGeminiModelChainConfiguration throws listing the missing role var (stubbed env)", async () => {
    const { assertGeminiModelChainConfiguration } = await import("../gemini/quotaConfig.ts");
    const prevAnalyzer = process.env.ANALYZER_MODEL_GEMINI;
    const prevPool = process.env.GEMINI_RATE_LIMIT_MODELS;
    delete process.env.ANALYZER_MODEL_GEMINI;
    process.env.GEMINI_RATE_LIMIT_MODELS = "";
    try {
      expect(() => assertGeminiModelChainConfiguration(["analyzer"])).toThrow(/ANALYZER_MODEL_GEMINI/);
    } finally {
      if (prevAnalyzer === undefined) delete process.env.ANALYZER_MODEL_GEMINI;
      else process.env.ANALYZER_MODEL_GEMINI = prevAnalyzer;
      if (prevPool === undefined) delete process.env.GEMINI_RATE_LIMIT_MODELS;

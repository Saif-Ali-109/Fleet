...
import { handleToolCall, isToolAllowedForRole, ALLOWED_TOOLS_PER_ROLE } from "../mcp/fleetServer.ts";
...
describe("MCP allowlist matrix — server-side handleToolCall enforcement", () => {
  it("rejects disallowed tool for analyzer with role-aware error", async () => {
    await expect(
      handleToolCall("create_pr", { owner: "o", repo: "r", head: "h", base: "b", title: "t", body: "b" }, "analyzer")
...
describe("MCP allowlist matrix — ALLOWED_TOOLS_PER_ROLE constant", () => {
  ...
  it("analyzer entry matches expected tools", () => {
    expect(ALLOWED_TOOLS_PER_ROLE.analyzer).toEqual(["get_issue", "get_issue_comments"]);
  });

...
function startCapturingCompletionsStub(text: string): Promise<{ baseUrl: string; requests: CapturedRequest[] }> {
  ...
  return new Promise((resolve) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      resolve({ baseUrl: `http://127.0.0.1:${typeof addr === "object" && addr ? addr.port : 0}/v1`, requests });
    });
  });
}
...
const RESUMED_MESSAGES = [
  ...
  { role: "user", content: "RESUMED_USER_TASK_MARKER" },
  {
    role: "assistant",
    content: null,
    tool_calls: [{ id: "call_9", type: "function", function: { name: "read", arguments: '{"path":"a.ts"}' } }],
  },
...
describe("worker resumeFrom checkpoint loading", () => {
  it("seeds the conversation from checkpoint.messages instead of constructing prompts", async () => {
    ...
    const { baseUrl, requests } = await startCapturingCompletionsStub("resumed fine");
    try {
      const run = runWorker(
        {
          role: "coder",
          task: "FRESH_TASK_PROMPT_MUST_BE_IGNORED",
          ctx: { ...makeJobCtx(root), resumeFrom: { messagesPath: checkpointPath } },
        },
        {
          ...process.env,
      ...
      expect(code).toBe(0);
      expect(run.lines[0]).toMatchObject({ t: "init", role: "coder", provider: "ollama", model: "qwen2.5-coder:7b" });
      expect(run.lines.map((ev) => ev.t)).toEqual(["init", "text", "result", "step_finish"]);
      expect(requests).toHaveLength(1);
      const sentMessages = requests[0]!.body.messages ?? [];
      expect(sentMessages).toEqual(JSON.parse(JSON.stringify(RESUMED_MESSAGES)));
      expect(JSON.stringify(sentMessages)).not.toContain("FRESH_TASK_PROMPT_MUST_BE_IGNORED");
  ...
  it("fails fast before any LLM call when the checkpoint file is missing", async () => {
    ...
    const { baseUrl, requests } = await startCapturingCompletionsStub("must never be called");
    try {
      ...
      const run = runWorker(
        {
          role: "coder",
          task: "Do work",
          ctx: { ...makeJobCtx(root), resumeFrom: { messagesPath: missingPath } },
        },
        {
          ...process.env,
          FLEET_PROVIDERS: "ollama",
      ...
      expect((await run.stderr).includes("resume checkpoint unreadable")).toBe(true);
      ...
      const run = runWorker(
        {
          role: "coder",
          task: "Do work",
          ctx: { ...makeJobCtx(root), resumeFrom: { messagesPath: corruptPath } },
        },
        {
          ...process.env,
          FLEET_PROVIDERS: "ollama",

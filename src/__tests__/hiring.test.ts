...
beforeAll(async () => {
	await pool.query("DELETE FROM worker_roles WHERE role_name LIKE $1", [
		`${TEST_PREFIX}%`,
	]);
});
...
describe("canHire", () => {
	it("allows an authorized role below limits", async () => {
		const result = await canHire(
			`${TEST_PREFIX}coder`,
			"opencode",
			hirePolicy(`${TEST_PREFIX}coder`),
		);
...
describe("updateWorkerStatus and retireWorker", () => {
	...
	it("updateWorkerStatus throws on an invalid status", async () => {
		...
		const id = await hireWorker(role, "claude", "m", hirePolicy(role));
		await expect(updateWorkerStatus(id, "bogus")).rejects.toThrow(

...
describe("parseProviderLine call counting", () => {
  it("counts tool_calls", () => {
    const s = freshAcc();
    parseProviderLine("gemini", { t: "tool_call", id: "1", name: "bash", input: { command: "ls" } }, s);
    parseProviderLine("gemini", { t: "tool_call", id: "2", name: "read", input: { path: "x" } }, s);
  ...
  it("counts load_skill as both a tool and a skill", () => {
    const s = freshAcc();
    parseProviderLine("gemini", { t: "tool_call", id: "1", name: "load_skill", input: { name: "x" } }, s);
  ...
  it("counts completed telemetry as model calls", () => {
    const s = freshAcc();
    parseProviderLine("gemini", { t: "telemetry", event: "provider_completion", status: "completed", model: "gemini-3.5-flash" }, s);
    parseProviderLine("gemini", { t: "telemetry", event: "provider_completion", status: "completed", model: "gemini-3.5-flash" }, s);
  ...
  it("does not count failed telemetry", () => {
    const s = freshAcc();
    parseProviderLine("gemini", { t: "telemetry", event: "provider_completion", status: "failed", model: "gemini-3.5-flash" }, s);
    expect(s.models).toBe(0);
  });

  it("does not count non-provider telemetry", () => {
    const s = freshAcc();
    parseProviderLine("gemini", { t: "telemetry", event: "rate_limit", status: "active" }, s);
    expect(s.models).toBe(0);
  });
});

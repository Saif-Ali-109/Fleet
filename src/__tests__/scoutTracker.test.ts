...
describe("ScoutTracker", () => {
	...
	it("counts the message.tool shape", () => {
		...
		const ev = {
			type: "message",
			message: { tool: "task", input: { description: "use scout" } },
		};
		...
		expect(
			t.observe("planner", {
				...
				part: {
					type: "tool_call",
					tool: "task",
					state: { input: { prompt: "Ask SCOUT for symbols" } },
				},
			}),
		).toBe(true);
		expect(t.total).toBe(1);
	...
	it("aggregates per-parent counts in summary", () => {
		...
		const ev = (role: string) => ({
			...
			part: {
				type: "tool_call",
				tool: "task",
				state: { input: { description: `scout via ${role}` } },
			},
		});
		...
		expect(t.summary()).toBe(
			"scout calls: 4 total (analyzer=1, planner=2, reviewer=1)",
		);
	});
	...
	it("never throws on garbage input", () => {
		const t = new ScoutTracker();
		expect(() =>
			t.observe("coder", {} as Record<string, unknown>),
		).not.toThrow();
		expect(() => t.observe("coder", { part: null })).not.toThrow();
		expect(() =>
			t.observe("coder", { part: { type: "tool_call" } }),
		).not.toThrow();
		expect(() =>
			t.observe("pr", undefined as unknown as Record<string, unknown>),
		).not.toThrow();
		expect(t.total).toBe(0);
		expect(t.summary()).toBe("scout calls: 0");
	});

...
describe("shouldSkipIssue", () => {
	...
	it("checks the done label via the issue view", async () => {
		...
		await shouldSkipIssue(base);
		const labelCall = execFileMock.mock.calls.find(
			(c) => (c[1] as string[])[0] === "issue",
		);
	...
	it("normalizes a mixed-case slug before every gh check", async () => {
		routeGh({ openPr: false, labels: [] });
		await shouldSkipIssue({ ...base, repoUrlOrSlug: "Owner/Repo" });
		const prCall = execFileMock.mock.calls.find(
			(c) => (c[1] as string[])[0] === "pr",
		);
		expect(prCall?.[1]).toEqual([
	...
	it("treats mixed-case slugs identically for skip decisions", async () => {
		routeGh({ openPr: true, labels: [] });
		await expect(
			shouldSkipIssue({
				repoUrlOrSlug: "Owner/REPO",
...
describe("hasOpenPrForBranch", () => {
	it("returns true when an open PR exists on the head branch", async () => {
		routeGh({ openPr: true, labels: [] });
		await expect(hasOpenPrForBranch("owner/repo", "fix-issue-5")).resolves.toBe(
			true,
		);
	});

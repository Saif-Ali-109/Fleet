...
import { isQuotaPaused, killActiveWorkers, parseTrace, readStderrTail, requestQuotaResume, resetWorkerAbort, runWorker } from "../agentRunner.ts";
import { RPD_EXHAUSTED } from "../fleet/quotaSignals.ts";
import { onQuotaEvent, resetQuotaEventListeners, type QuotaEvent } from "../fleet/quotaEvents.ts";
import type { AgentResult, RunContext, Role, RolePolicy } from "../types.ts";
...
function makeGeminiPolicy(): RolePolicy {
  return { role: "coder", model: "gemini-test-primary", fallbacks: ["gemini-test-fallback"] };
}
...
afterEach(() => {
  ...
  while (extraTmpDirs.length) rmSync(extraTmpDirs.pop()!, { recursive: true, force: true });
 ...
 */
async function settleUnderFakeTimers(pending: Promise<AgentResult>): Promise<AgentResult> {
  let settled = false;
  ...
  while (!settled) {
    ...
    if (performance.now() - started > 60000) throw new Error("runWorker did not settle under fake timers");
  }
...
describe("runWorker", () => {
  ...
  it("keeps an explicitly selected provider authoritative over FLEET_PROVIDERS", async () => {
    ...
    const ctx = makeRealCtx();
    const res = await runWorker("coder", "selected provider", { ...ctx, provider: "gemini" }, makePolicy(), {});

    expect(res.ok).toBe(true);
    expect(res.provider).toBe("gemini");
    expect(res.attempts).toEqual([{ model: "fake-model", ok: true, provider: "gemini" }]);
  ...
  it("does not validate Gemini quota configuration for an explicitly selected non-Gemini provider", async () => {
    ...
    const ctx = makeRealCtx();
    const res = await runWorker("coder", "selected ollama", { ...ctx, provider: "ollama" }, makePolicy(), {});
  ...
  it("fails clearly instead of falling back when selected provider has no key", async () => {
    ...
    process.env.FLEET_PROVIDERS = "ollama";
    const ctx = makeRealCtx();
    const res = await runWorker("coder", "missing key", { ...ctx, provider: "gemini" }, makePolicy(), {});

    expect(res.ok).toBe(false);
    expect(res.error).toBe('selected provider "gemini" has no key configured');
    expect(res.attempts).toEqual([{ model: "none", ok: false, error: 'selected provider "gemini" has no key configured' }]);
...
describe("runWorker Gemini quota chain walk", () => {
  it("switches to the fallback on an rpm block and succeeds without a recovery event", async () => {
    ...
    const res = await runWorker("coder", "quota switch", ctx, makeGeminiPolicy(), {});

    expect(res.ok).toBe(true);
    expect(res.model).toBe("gemini-test-fallback");
    expect(res.attempts).toEqual([
      { model: "gemini-test-primary", ok: false, error: "GEMINI_RATE_LIMIT_SWITCH:rpm:4200", provider: "gemini" },
      { model: "gemini-test-fallback", ok: true, provider: "gemini" },
    ]);
    expect(events).toEqual([
      { type: "model_switch", role: "coder", provider: "gemini", fromModel: "gemini-test-primary", toModel: "gemini-test-fallback", block: "rpm", waitMs: 4200 },
    ]);
  }, 30000);
  ...
  it("restarts the chain from the top after a capped sleep until the primary recovers", async () => {
    ...
    try {
      ...
      expect(readFileSync(join(process.env.FAKE_QUOTA_DIR!, "models.log"), "utf8").trim().split("\n")).toEqual([
        "gemini-test-primary",
      ...
      expect(res.attempts!.map((a) => `${a.model}:${a.ok ? "ok" : a.error}`)).toEqual([
      ...
      expect(events).toEqual([
        { type: "model_switch", role: "coder", provider: "gemini", fromModel: "gemini-test-primary", toModel: "gemini-test-fallback", block: "rpm", waitMs: 7000 },
        { type: "model_switch", role: "coder", provider: "gemini", fromModel: "gemini-test-primary", toModel: "gemini-test-fallback", block: "rpm", waitMs: 400 },
        { type: "model_recovered", role: "coder", provider: "gemini", model: "gemini-test-primary" },
      ]);
      // Two capped waits (1000ms + 400ms), polled in <=100ms slices: exactly
      // fourteen 100ms sleeps proves the min(waitMs)-vs-ceiling math.
      const sleeps = setTimeoutSpy.mock.calls.map((c) => c[1] as number).filter((ms) => ms === 100);
  ...
  it("pauses on all-RPD instead of failing, then resumes from the chain top seeded with the checkpoint", async () => {
    ...
    const pending = runWorker("coder", "quota pause", ctx, makeGeminiPolicy(), {});
    ...
    writeFileSync(checkpointPath, JSON.stringify({ role: "coder", model: "gemini-test-primary", chainIndex: 0, messages: [], savedAt: new Date().toISOString() }), "utf8");
    ...
    expect(res.model).toBe("gemini-test-primary");
    expect(res.attempts).toEqual([
      { model: "gemini-test-primary", ok: false, error: RPD_EXHAUSTED, provider: "gemini" },
      { model: "gemini-test-fallback", ok: false, error: RPD_EXHAUSTED, provider: "gemini" },
    ...
    const resumeLog = readFileSync(join(process.env.FAKE_QUOTA_DIR!, "resume.log"), "utf8").trim().split("\n");
  ...
  it("re-enters the pause (loop-safe) when the resumed key is exhausted again", async () => {
    ...
    const pending = runWorker("coder", "quota re-pause", ctx, makeGeminiPolicy(), {});
    await waitFor(() => isQuotaPaused(), "first pause");
    expect(requestQuotaResume()).toBe(true);
    // The second exhaustion proves the walk actually restarted and parked again.
    const exhaustions = (): QuotaEvent[] => events.filter((e) => e.type === "all_models_exhausted");
  ...
  it("finalizes failed through the normal path when the user Stops during the pause", async () => {
    ...
    const pending = runWorker("coder", "pause abort", ctx, makeGeminiPolicy(), {});
    await waitFor(() => isQuotaPaused(), "quota pause entry");
  ...
  it("restarts from the top without ctx.resumeFrom when no checkpoint exists on disk", async () => {
    ...
    const pending = runWorker("coder", "pause no checkpoint", ctx, makeGeminiPolicy(), {});
    await waitFor(() => isQuotaPaused(), "quota pause entry");
    expect(requestQuotaResume()).toBe(true);
    const res = await pending;
  ...
  it("stops immediately with the raw error on a non-temporary unknown failure", async () => {
    ...
    const res = await runWorker("coder", "unknown error", ctx, makeGeminiPolicy(), {});

    expect(res.ok).toBe(false);
    expect(res.error).toBe("kaboom");
    expect(res.attempts).toEqual([
      { model: "gemini-test-primary", ok: false, error: "kaboom", provider: "gemini" },
    ]);
    expect(events).toEqual([]);
  }, 30000);
  ...
  it("falls back on a 503 HTTP status error and succeeds on the next model", async () => {
    ...
    installQuotaWorker([
      { error: '503 [{"error":{"code":503,"message":"Service Unavailable","status":"UNAVAILABLE"}}]' },
      {},
    ]);
    const ctx = makeRealCtx();
    const events = collectQuotaEvents();

    const policy: RolePolicy = { role: "coder", model: "m-a", fallbacks: ["m-b", "m-c"] };
    const res = await runWorker("coder", "http 503 fallback", ctx, policy, {});

    expect(res.ok).toBe(true);
    expect(res.model).toBe("m-b");
    expect(res.attempts).toEqual([
      { model: "m-a", ok: false, error: '503 [{"error":{"code":503,"message":"Service Unavailable","status":"UNAVAILABLE"}}]', provider: "gemini" },
      { model: "m-b", ok: true, provider: "gemini" },
    ]);
    expect(events).toEqual([
      { type: "model_switch", role: "coder", provider: "gemini", fromModel: "m-a", toModel: "m-b", block: "http", waitMs: 0 },
    ]);
    expect(readFileSync(join(process.env.FAKE_QUOTA_DIR!, "models.log"), "utf8").trim().split("\n")).toEqual(["m-a", "m-b"]);
  }, 30000);
  ...
  it("exhausts all models when every one returns a 503 HTTP status error", async () => {
    ...
    const policy: RolePolicy = { role: "coder", model: "m-a", fallbacks: ["m-b"] };
    const res = await runWorker("coder", "all 503 exhaust", ctx, policy, {});

    expect(res.ok).toBe(false);
    expect(res.error).toBe('503 [{"error":{"code":503,"message":"Service Unavailable","status":"UNAVAILABLE"}}]');
  ...
  it("falls back on a 500 HTTP status error the same way as 503", async () => {
    ...
    installQuotaWorker([
      { error: '500 [{"error":{"code":500,"message":"Internal Server Error","status":"INTERNAL"}}]' },
      {},
    ]);
    const ctx = makeRealCtx();
    const events = collectQuotaEvents();

    const policy: RolePolicy = { role: "coder", model: "m-x", fallbacks: ["m-y"] };
    const res = await runWorker("coder", "http 500 fallback", ctx, policy, {});

    expect(res.ok).toBe(true);
    expect(res.model).toBe("m-y");
    expect(res.attempts).toEqual([
      { model: "m-x", ok: false, error: '500 [{"error":{"code":500,"message":"Internal Server Error","status":"INTERNAL"}}]', provider: "gemini" },
      { model: "m-y", ok: true, provider: "gemini" },
    ]);
    expect(events).toEqual([
      { type: "model_switch", role: "coder", provider: "gemini", fromModel: "m-x", toModel: "m-y", block: "http", waitMs: 0 },
    ]);
  }, 30000);
  ...
  it("keeps the openrouter walk single-model and quota-event-free", async () => {
    ...
    const res = await runWorker("coder", "openrouter walk", { ...ctx, provider: "openrouter" }, makeGeminiPolicy(), {});

    expect(res.ok).toBe(true);
    expect(res.provider).toBe("openrouter");
    expect(res.attempts).toEqual([{ model: "fake-model", ok: true, provider: "openrouter" }]);
    expect(events).toEqual([]);
  }, 30000);
});

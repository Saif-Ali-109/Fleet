...
function resolveWith(stdout: string): void {
  execFileMock.mockImplementation(
    (_file: string, _args: string[], _opts: unknown, cb: (err: Error | null, out?: unknown, errOut?: string) => void) => {
...
const ghCalls = (): { file: string; args: string[] }[] =>
  execFileMock.mock.calls.map((c) => ({ file: c[0] as string, args: c[1] as string[] }));
...
describe("issue lifecycle gh helpers", () => {
  ...
  it("commentOnIssue posts the body via gh issue comment --body", async () => {
    resolveWith("");
    await commentOnIssue("owner", "repo", 42, "Started managed run `run-1` (backend: opencode).");
  ...
  it("hasIssueLabel reports true when the label is present", async () => {
    resolveWith(JSON.stringify({ labels: [{ name: "multi-orch/in-progress" }, { name: "bug" }] }));
    await expect(hasIssueLabel("owner", "repo", 42, ISSUE_LABEL_IN_PROGRESS)).resolves.toBe(true);
  });

  it("hasIssueLabel reports false when the label is absent", async () => {
    resolveWith(JSON.stringify({ labels: [{ name: "bug" }] }));
    await expect(hasIssueLabel("owner", "repo", 42, ISSUE_LABEL_IN_PROGRESS)).resolves.toBe(false);
  });
...
describe("non-fatal behavior", () => {
  it("addIssueLabel never throws when gh fails — it only warns", async () => {
    ...
    try {
      await expect(addIssueLabel("owner", "repo", 42, ISSUE_LABEL_DONE)).resolves.toBeUndefined();
  ...
  it("commentOnIssue never throws when gh fails — it only warns", async () => {
    ...
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    try {
      await expect(commentOnIssue("owner", "repo", 42, "Managed run `x` failed.")).resolves.toBeUndefined();
      expect(warn).toHaveBeenCalled();
    } finally {
      warn.mockRestore();
    }
  ...
  it("ensureLabels continues after one failing label and never throws", async () => {
    ...
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    try {
      await expect(ensureLabels("owner", "repo", [ISSUE_LABEL_IN_PROGRESS, ISSUE_LABEL_DONE])).resolves.toBeUndefined();

...
async function connect(role: string, opts?: { fakeGh?: boolean }): Promise<Conn> {
  ...
  const client = new Client({ name: "fleet-mcp-test", version: "1.0.0" }, { capabilities: {} });
...
describe("MCP stdio integration — denied role (coder) over real server", () => {
  it("listTools yields no tools for denied role coder", { timeout: 30000 }, async () => {
  ...
  it("callTool get_issue is denied for coder", { timeout: 30000 }, async () => {
    const { client } = await connect("coder");
    const result = await client.callTool({ name: "get_issue", arguments: { owner: "o", repo: "r", number: 1 } });
  ...
  it("callTool create_pr is denied for coder even with spoofed _meta role analyzer", { timeout: 30000 }, async () => {
    const { client } = await connect("coder");
    const result = await client.callTool({
      name: "create_pr",
      arguments: { owner: "o", repo: "r", head: "h", base: "b", title: "t", body: "b" },
      _meta: { role: "analyzer" },
    });
    expect(result.isError).toBe(true);
    expect(resultText(result.content)).toContain(
...
describe("MCP stdio integration — unknown/unmapped role denies everything", () => {
  it("listTools yields nothing and callTool is denied for unmapped role intruder", { timeout: 30000 }, async () => {
    const conn = await connect("intruder");
    const list = await conn.client.listTools();
    expect(list.tools).toEqual([]);

    const call = await conn.client.callTool({ name: "get_issue", arguments: { owner: "o", repo: "r", number: 1 } });
    expect(call.isError).toBe(true);
    expect(
      resultText(call.content)
    ).toContain("not allowed for role intruder");
  });
});

describe("MCP stdio integration — allowed role (analyzer) positive control", () => {
  it("listTools exposes exactly the analyzer allowlist", { timeout: 30000 }, async () => {
    const { client } = await connect("analyzer");
    const result = await client.listTools();
    expect(result.tools.map((t) => t.name)).toEqual(["get_issue", "get_issue_comments"]);
  });

  it("callTool get_issue executes through gh-backed handler", { timeout: 30000 }, async () => {
    const { client } = await connect("analyzer", { fakeGh: true });
    const result = await client.callTool({ name: "get_issue", arguments: { owner: "o", repo: "r", number: 7 } });
  ...
  it("callTool create_pr stays denied for analyzer", { timeout: 30000 }, async () => {
    const { client } = await connect("analyzer");
    const result = await client.callTool({
      name: "create_pr",
      arguments: { owner: "o", repo: "r", head: "h", base: "b", title: "t", body: "b" },

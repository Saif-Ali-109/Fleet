...
describe("verifyWebhookSignature", () => {
	...
	it("returns false without throwing for a truncated / garbage-length header", () => {
		...
		expect(() => verifyWebhookSignature(body, garbage, secret)).not.toThrow();
...
describe("parseIssueEvent", () => {
	...
	it("returns null when issue is missing", () => {
		expect(
			parseIssueEvent({ action: "opened", repository: { full_name: "o/r" } }),
		).toBeNull();
	});

	it("returns null when repository is missing", () => {
		expect(
			parseIssueEvent({ action: "opened", issue: { number: 1 } }),
		).toBeNull();
	});

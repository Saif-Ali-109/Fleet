...
type PhaseName = "idle" | "analyze" | "plan" | "implement" | "review" | "pr" | "paused" | "done" | "aborted" | "failed";
...
export function renderDashboard(d: DashboardState): string {
  const rows = ROLES.map((r) => {
    ...
    const bar = state === "done" ? renderBar(1) : state === "running" ? renderBar(0.5) : renderBar(0);
    const meta =
  ...
  if (d.totals) {
    ...
    if (d.totals.tokens) totalsParts.push(`${d.totals.tokens.toLocaleString()} tok`);
  }

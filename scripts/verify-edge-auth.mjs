...
try {
	...
	console.error(
		"  openssl genpkey -algorithm Ed25519 -out ~/.config/fleet/keys/ed25519-private.pem",
	);
	console.error(
		"  openssl pkey -in ~/.config/fleet/keys/ed25519-private.pem -pubout -out ~/.config/fleet/keys/ed25519-public.pem",
	);
	process.exit(1);
}
...
if (!subtle) {
	console.error(
		"✗ globalThis.crypto.subtle is not available (Node.js 20+ or browser required)",
	);
	process.exit(1);
}
...
const [hdrB64, payloadB64, _sigB64] = token.split(".");
...
try {
	...
	if (res.ok) {
		...
		console.log(
			`  ✓ Server responded ${res.status} — ${models.length} model(s) visible`,
		);
		if (models.length > 0) {
			console.log(
		...
		console.log(
			`    (JWT auth may be working but server rejected the request)`,
		);
	}
} catch (err) {
	if (err.name === "TimeoutError" || err.name === "AbortError") {
		console.log(
			`  ⚠ Server did not respond within 3s — is model-api-proxy running?`,
		);
	} else if (err.cause?.code === "ECONNREFUSED") {
		console.log(
			`  ⚠ Connection refused — model-api-proxy is not running on this port`,
		);
	} else {
		console.log(`  ⚠ ${err.message}`);
	}
	console.log(
		`    (key signing itself worked; server check is informational only)`,
	);
}
